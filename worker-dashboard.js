...
.wd-sum-ok-dot { width:7px; height:7px; border-radius:50%; flex-shrink:0; }
  ...
  function myTodaySales() {
    return (_sc().byUid[window.CU?.uid] || []).filter(s => s.date === today());
  }

  function myAllSales() {
    return (_sc().byUid[window.CU?.uid] || []);
  }

  function _sc() { return window._salesCache || window._sc || { byUid:{}, byShop:{}, arr:[], dueByShop:{}, totalByShop:{} }; }

  function myTodayAtt() {
    const att = Object.values(window.allAttendance || {});
    return att.find(a => a.uid === window.CU?.uid && a.date === today());
  }
  ...
  function myDue() {
    return myAllSales().filter(s => s.due > 0).reduce((a, s) => a + (s.due || 0), 0);
  }

  function myDueShops() {
    return myAllSales().filter(s => s.due > 0).length;
  }

  function myTodayRpl() {
    const sales = (window.allSales || {});
    return Object.values(sales).filter(s =>
      s.uid === window.CU?.uid && s.date === today() && s.replacement
    );
  }
  ...
  function todayVisits() {
    ...
    const visited = myTodaySales().map(s => s.shopId).filter(Boolean);
  ...
  function render() {
    ...
    const sal       = myTodaySales();
    const totalSale = sal.reduce((a, s) => a + (s.total || 0), 0);
    const totalCash = sal.filter(s => s.payType === 'paid').reduce((a, s) => a + (s.total || 0), 0);
    ...
    const rplSales  = myTodayRpl();
    const rplTotal  = rplSales.reduce((a, s) => a + ((s.replacement?.totalIn || 0) - (s.replacement?.totalOut || 0)), 0);
    ...
    <div class="wd-content">
      ...
      <div>
        <div class="wd-sec-hdr">
          <div class="wd-sec-title">📅 আজকের পারফরম্যান্স</div>
          <div class="wd-sec-badge">${new Date().toLocaleDateString('bn-BD',{day:'numeric',month:'short'})}</div>
        </div>
      ...
      <div class="wd-rpl-card">
        ...
        ${rplSales.map(s => {
          const rpl = s.replacement || {};
          return `
          <div class="wd-rpl-item">
            <div style="font-size:16px;">🔄</div>
            <div style="flex:1;">
              <div style="font-size:12px;font-weight:600;color:var(--text);">${s.shop || ''} — ${bnN(rpl.qtyIn||0)} পিস</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px;">দাম: ${bn(rpl.totalIn||0)}</div>
            </div>
            <div style="text-align:right;">
              <div class="wd-rpl-tag wrt-in">ফেরত</div>
              <div style="font-family:'Sora',sans-serif;font-size:12px;font-weight:800;color:#a78bfa;margin-top:3px;">${bn(rpl.totalIn||0)}</div>
            </div>
          </div>`;
        }).join('')}
        <div class="wd-rpl-total">
          <div class="wd-rpl-trow">
            ...
            <span style="color:#a78bfa;font-weight:700;">${bn(rplSales.reduce((a,s)=>a+(s.replacement?.totalIn||0),0))}</span>
          </div>
          <div class="wd-rpl-trow">
            <span style="color:var(--muted);font-size:11px;">বদলে দেওয়া মোট</span>
            <span style="color:#34d399;font-weight:700;">${bn(rplSales.reduce((a,s)=>a+(s.replacement?.totalOut||0),0))}</span>
          </div>
        ...
        <div class="wd-sum-grid">
          ...
          <div class="wd-sum-item">
            ...
            <div>
              ...
              <div class="wd-sum-val wsv-p">${bnN(rplSales.length)} পিস</div>
            </div>
          </div>
        </div>
