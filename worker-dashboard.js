...
.wd-sum-ok-dot { width:7px; height:7px; border-radius:50%; flex-shrink:0; }
  ...
  function myDue() {
    const all = Object.values(window.allSales || {});
    return all.filter(s => s.uid === window.CU?.uid && s.due > 0)
              .reduce((a, s) => a + (s.due || 0), 0);
  }

  function myDueShops() {
    const all = Object.values(window.allSales || {});
    return all.filter(s => s.uid === window.CU?.uid && s.due > 0).length;
  }
  ...
  async function render() {
    ...
    // ✅ Firebase থেকে fresh sales data নিই
    try {
      const freshSales = await fbGet('sales');
      if (freshSales) {
        ...
        if (typeof window.computeSalesCache === 'function') window.computeSalesCache();
          ...
          // manual _sc update
          const arr = Object.entries(freshSales).map(([id,s])=>({...s,_id:id}));
          if (window._sc) {
            window._sc.arr = arr;
            window._sc.byUid = {};
            arr.forEach(s => {
      ...
      // Fresh replacement data
      const freshRpl = await fbGet('replacements');
      if (freshRpl !== null) window.allReplacements = freshRpl || {};
