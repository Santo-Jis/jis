...
.wd-sum-ok-dot { width:7px; height:7px; border-radius:50%; flex-shrink:0; }
  ...
  function myPeriodRpl() {
    // replacement.js থেকে allReplacements
    const all = Object.values(window.allReplacements || {});
    const uid = window.CU?.uid;
    if (_wdPeriod === '1') {
      return all.filter(r => r.uid === uid && r.date === today());
    }
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - parseInt(_wdPeriod));
    const cutoffStr = cutoff.toISOString().split('T')[0];
    return all.filter(r => r.uid === uid && r.date >= cutoffStr);
  }
  ...
  function render() {
    ...
    // ✅ নগদ: payStatus='paid' অথবা due=0
    const totalCash = sal.filter(s => s.payStatus === 'paid' || s.due === 0).reduce((a, s) => a + (s.total || 0), 0);
    ...
    <div class="wd-content">
      ...
      <div class="wd-rpl-card">
        ...
        ${rplSales.map(r => {
          const retQty = (r.retItems||[]).reduce((a,i)=>a+(i.qty||0),0);
          const givQty = (r.givItems||[]).reduce((a,i)=>a+(i.qty||0),0);
          return `
          <div class="wd-rpl-item">
            <div style="font-size:16px;">🔄</div>
            <div style="flex:1;">
              <div style="font-size:12px;font-weight:600;color:var(--text);">${r.shopName||''}</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px;">
                ফেরত: ${bnN(retQty)} পিস · ${bn(r.retTotal||0)}
                ${givQty>0?` | দিলাম: ${bnN(givQty)} পিস · ${bn(r.givTotal||0)}`:''}
              </div>
            </div>
            <div style="text-align:right;">
              <div class="wd-rpl-tag ${(r.diff||0)>0?'wrt-out':'wrt-in'}">${(r.diff||0)>0?'বাকি':'সমান'}</div>
              <div style="font-family:'Sora',sans-serif;font-size:12px;font-weight:800;color:#a78bfa;margin-top:3px;">${bn(r.retTotal||0)}</div>
            </div>
          </div>`;
        }).join('')}
