...
<div class="rpl-wrap">
  ...
  function loadSelects() {
    ...
    // দেওয়া মাল optional — auto add করব না
  }
  ...
  window.rplSave = async function() {
    ...
    // দেওয়া মাল optional — না দিলেও চলবে

    const hasInvalidRet = _retItems.some(i => !i.prodId || i.qty < 1);
    if (_retItems.length > 0 && _retItems.some(i => !i.prodId || i.qty < 1)) { toast('ফেরত পণ্যের তথ্য সম্পূর্ণ করুন!', true); return; }
    if (_givItems.length > 0 && _givItems.some(i => !i.prodId || i.qty < 1)) { toast('দেওয়া পণ্যের তথ্য সম্পূর্ণ করুন!', true); return; }
    ...
    try {
      ...
      if (shopSales.length > 0) {
        ...
        if (_givItems.length === 0 && retTotal > 0) {
          // দেওয়া মাল নেই → মূল বিক্রয়ের due থেকে ফেরত বিয়োগ
          const newDue = Math.max(0, (ls.due || 0) - retTotal);
          const newTotal = Math.max(0, (ls.total || 0) - retTotal);
          await fbUpdate(`sales/${lid}`, { due: newDue, total: newTotal, rplDeduct: retTotal });
          toast(`✅ সংরক্ষিত — বিল থেকে ${bn(retTotal)} বিয়োগ হয়েছে`);

        } else if (settleMethod === 'due' && diff !== 0) {
          const newDue = (ls.due || 0) + diff;
          await fbUpdate(`sales/${lid}`, { due: newDue, hasCredit: newDue < 0 });
          toast(diff > 0
            ? `✅ সংরক্ষিত — ${bn(diff)} আমরা দোকানের কাছে পাবো`
            : `✅ সংরক্ষিত — ${bn(Math.abs(diff))} দোকান আমাদের কাছে পাবে`);
        } else {
...
<script>window.onload=()=>window.print()</script>
  ...
  waitForApp(() => {
    ...
    window.generateInvoice = function(saleId) {
      ...
      setTimeout(() => {
        ...
        // ── rplDeduct থাকলে মূল invoice এর total section এ ব্যাখ্যা যোগ করি ──
        if (sale?.rplDeduct > 0 && !ic.querySelector('#rpl-deduct-note')) {
          const deductNote = document.createElement('div');
          deductNote.id = 'rpl-deduct-note';
          deductNote.style.cssText = 'margin-bottom:10px';
          deductNote.innerHTML = `
            <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:9px 12px">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
                <span style="font-size:12px;font-weight:700;color:#15803d">🔄 রিপ্লেসমেন্ট বিয়োগ</span>
                <span style="font-size:13px;font-weight:800;color:#15803d">- ৳${Math.round(sale.rplDeduct)}</span>
              </div>
              <div style="font-size:10px;color:#166534">ফেরত মালের দাম মূল বিল থেকে বাদ দেওয়া হয়েছে</div>
            </div>`;

          // মোট পরিশোধযোগ্য section এর আগে insert করি
          const totalSection = ic.querySelector('[style*="059669"]') ||
                               ic.querySelector('[style*="পরিশোধযোগ্য"]') ||
                               ic.querySelector('div[style*="font-size:16px"]');
          if (totalSection?.parentNode) {
            totalSection.parentNode.insertBefore(deductNote, totalSection);
          } else {
            // fallback — invoice content এর শেষ বড় section এর আগে
            const sections = ic.querySelectorAll('div[style*="background:#f8fafc"]');
            const lastSection = sections[sections.length - 1];
            if (lastSection) lastSection.parentNode.insertBefore(deductNote, lastSection);
          }
        }

        // আগের replacement section থাকলে সরাও
        ...
        rplSection.style.cssText = 'margin-top:0;padding:0';

        const saleDue = sale?.due || 0;
        // deduct case: ফেরত মাল বিল থেকে বাদ — netDue = saleDue - retTotal (min 0)
        // normal case: netDue = saleDue + diff
        const isDeduct = givItems.length === 0 && (rpl.settleMethod === 'deduct' || !rpl.givTotal);
        const netDue  = isDeduct ? Math.max(0, saleDue - (rpl.retTotal||0)) : saleDue + diff;
        const netAbs  = Math.abs(netDue);

        // তারিখ ও সময় format helper
        const fmtDT = ts => ts ? new Date(ts).toLocaleString('bn-BD',{
          day:'numeric', month:'long', year:'numeric',
          hour:'2-digit', minute:'2-digit'
        }) : '';

        rplSection.innerHTML = `
          <div style="margin-top:0;border-top:2px dashed #cbd5e1;padding-top:16px;font-family:'Hind Siliguri',Arial,sans-serif">

            <!-- শিরোনাম -->
            <div style="text-align:center;margin-bottom:14px">
              <div style="font-size:14px;font-weight:800;color:#1e293b;letter-spacing:-.3px">🔄 মাল ফেরত ও বদলির হিসাব</div>
              <div style="font-size:10px;color:#94a3b8;margin-top:3px">${fmtDT(rpl.ts)}</div>
            </div>
            ...
            <div style="position:relative;padding-left:22px">
              ...
              <div style="position:relative;margin-bottom:16px">
                <div style="position:absolute;left:-22px;top:3px;width:14px;height:14px;border-radius:50%;background:#dc2626;border:2px solid white;box-shadow:0 0 0 2px #dc2626"></div>
                <div style="font-size:12px;font-weight:800;color:#dc2626;margin-bottom:6px">আপনি ফেরত দিলেন</div>
                <div style="background:#fff5f5;border:1px solid #fecaca;border-radius:10px;padding:10px">
                  ${retItems.map((i,idx) => `
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;${idx<retItems.length-1?'border-bottom:1px solid #fecaca':''}">
                      <div>
                        <div style="font-size:12px;font-weight:700;color:#1e293b">${i.prodName}</div>
                        <div style="font-size:10px;color:#94a3b8">${i.qty} পিস × ৳${i.price}</div>
                      </div>
                      <div style="font-size:13px;font-weight:800;color:#dc2626">৳${i.qty*i.price}</div>
                    </div>`).join('')}
                  <div style="display:flex;justify-content:space-between;margin-top:7px;padding-top:6px;border-top:1.5px solid #fca5a5">
                    <span style="font-size:11px;font-weight:700;color:#dc2626">মোট ফেরত</span>
                    <span style="font-size:14px;font-weight:900;color:#dc2626">৳${Math.round(rpl.retTotal||0)}</span>
                  </div>
                </div>
              </div>

              <!-- ধাপ ২: কী নতুন নিলেন (optional) -->
              ${givItems.length > 0 ? `
              <div style="position:relative;margin-bottom:16px">
                <div style="position:absolute;left:-22px;top:3px;width:14px;height:14px;border-radius:50%;background:#16a34a;border:2px solid white;box-shadow:0 0 0 2px #16a34a"></div>
                <div style="font-size:12px;font-weight:800;color:#16a34a;margin-bottom:6px">আপনি নতুন মাল নিলেন</div>
                <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:10px;padding:10px">
                  ${givItems.map((i,idx) => `
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;${idx<givItems.length-1?'border-bottom:1px solid #bbf7d0':''}">
                      <div>
                        <div style="font-size:12px;font-weight:700;color:#1e293b">${i.prodName}</div>
                        <div style="font-size:10px;color:#94a3b8">${i.qty} পিস × ৳${i.price}</div>
                      </div>
                      <div style="font-size:13px;font-weight:800;color:#16a34a">৳${i.qty*i.price}</div>
                    </div>`).join('')}
                  <div style="display:flex;justify-content:space-between;margin-top:7px;padding-top:6px;border-top:1.5px solid #86efac">
                    <span style="font-size:11px;font-weight:700;color:#16a34a">মোট নতুন মাল</span>
                    <span style="font-size:14px;font-weight:900;color:#16a34a">৳${Math.round(rpl.givTotal||0)}</span>
                  </div>
                </div>
              </div>` : `
              <div style="position:relative;margin-bottom:16px">
                <div style="position:absolute;left:-22px;top:3px;width:14px;height:14px;border-radius:50%;background:#f59e0b;border:2px solid white;box-shadow:0 0 0 2px #f59e0b"></div>
                <div style="font-size:12px;font-weight:800;color:#d97706;margin-bottom:6px">বিলে সমন্বয়</div>
                <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:10px;padding:10px;font-size:12px;color:#92400e">
                  📋 নতুন মাল নেওয়া হয়নি — ফেরত মালের দাম <strong>৳${Math.round(rpl.retTotal||0)}</strong> মূল বিলের সাথে সমন্বয় হবে
                </div>
              </div>`}

              <!-- ধাপ ৩: চূড়ান্ত হিসাব -->
              <div style="position:relative">
                <div style="position:absolute;left:-22px;top:3px;width:14px;height:14px;border-radius:50%;background:${netDue>0?'#1d4ed8':netDue<0?'#d97706':'#059669'};border:2px solid white;box-shadow:0 0 0 2px ${netDue>0?'#1d4ed8':netDue<0?'#d97706':'#059669'}"></div>
                <div style="font-size:12px;font-weight:800;color:#1e293b;margin-bottom:6px">সব মিলিয়ে হিসাব</div>
                <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:10px">

                  <!-- হিসাবের ধাপগুলো -->
                  ${isDeduct ? `
                  <div style="display:flex;justify-content:space-between;font-size:11px;color:#64748b;padding:4px 0;border-bottom:1px solid #f1f5f9">
                    <span>বাকি ছিল</span>
                    <span style="font-weight:700;color:#dc2626">৳${Math.round(saleDue)}</span>
                  </div>` : saleDue > 0 ? `
                  <div style="display:flex;justify-content:space-between;font-size:11px;color:#64748b;padding:4px 0;border-bottom:1px solid #f1f5f9">
                    <span>আগে থেকে বাকি ছিল</span>
                    <span style="font-weight:700;color:#dc2626">+ ৳${Math.round(saleDue)}</span>
                  </div>` : ''}
                  <div style="display:flex;justify-content:space-between;font-size:11px;color:#64748b;padding:4px 0;border-bottom:1px solid #f1f5f9">
                    <span>আপনি ফেরত দিলেন</span>
                    <span style="font-weight:700;color:#16a34a">- ৳${Math.round(rpl.retTotal||0)}</span>
                  </div>
                  ${givItems.length > 0 ? `
                  <div style="display:flex;justify-content:space-between;font-size:11px;color:#64748b;padding:4px 0;border-bottom:1.5px solid #cbd5e1;margin-bottom:8px">
                    <span>আপনি নতুন মাল নিলেন</span>
                    <span style="font-weight:700;color:#dc2626">+ ৳${Math.round(rpl.givTotal||0)}</span>
                  </div>` : `
                  <div style="display:flex;justify-content:space-between;font-size:11px;color:#059669;padding:4px 0;border-bottom:1.5px solid #cbd5e1;margin-bottom:8px;font-weight:700">
                    <span>✓ ফেরত মাল বিল থেকে বাদ</span>
                    <span>- ৳${Math.round(rpl.retTotal||0)}</span>
                  </div>`}

                  <!-- ফলাফল -->
                  <div style="padding:12px;border-radius:10px;text-align:center;background:${isDeduct?'#f0fdf4':netDue>0?'#eff6ff':netDue<0?'#fffbeb':'#f0fdf4'};border:1.5px solid ${isDeduct?'#86efac':netDue>0?'#93c5fd':netDue<0?'#fde68a':'#86efac'}">
                    ${isDeduct ? `
                      ${netDue === 0 ? `
                        <div style="font-size:14px;font-weight:800;color:#059669">✅ সমন্বয় সম্পন্ন!</div>
                        <div style="font-size:11px;color:#059669;margin-top:4px">ফেরত মাল বিল থেকে বাদ হয়েছে — কোনো বাকি নেই</div>
                      ` : `
                        <div style="font-size:11px;color:#64748b;margin-bottom:4px">ফেরত মালের পর বাকি আছে</div>
                        <div style="font-size:32px;font-weight:900;color:#1d4ed8;line-height:1">৳${netDue}</div>
                        <div style="font-size:10px;color:#3b5fc0;margin-top:6px;padding:5px 8px;background:white;border-radius:6px">
                          ফেরত মাল ৳${Math.round(rpl.retTotal||0)} বিল থেকে বাদ দেওয়া হয়েছে
                        </div>
                      `}
                    ` : netDue > 0 ? `
                      <div style="font-size:11px;color:#64748b;margin-bottom:4px">আপনাকে এই টাকা দিতে হবে</div>
                      <div style="font-size:32px;font-weight:900;color:#1d4ed8;line-height:1">৳${netAbs}</div>
                      <div style="font-size:10px;color:#3b5fc0;margin-top:6px;padding:5px 8px;background:white;border-radius:6px">
                        পরেরবার মাল নেওয়ার সময় পরিশোধ করলেই হবে
                      </div>
                    ` : netDue < 0 ? `
                      <div style="font-size:11px;color:#92400e;margin-bottom:4px">আমরা আপনাকে এই পরিমাণ দিতে বাধ্য</div>
                      <div style="font-size:32px;font-weight:900;color:#d97706;line-height:1">৳${netAbs}</div>
                      <div style="font-size:10px;color:#92400e;margin-top:6px;padding:5px 8px;background:white;border-radius:6px">
                        📦 আমরা নগদ টাকা দিই না<br>পরেরবার মাল নেওয়ার সময় <strong>৳${netAbs}</strong> এর মাল বিনামূল্যে পাবেন
                      </div>
                    ` : `
                      <div style="font-size:14px;font-weight:800;color:#059669">✅ সব হিসাব চুকে গেছে!</div>
                      <div style="font-size:10px;color:#64748b;margin-top:4px">কোনো বাকি নেই — দুই পক্ষ সমান</div>
                    `}
                  </div>
                </div>
              </div>
            ...
            ${rpl.note ? `<div style="margin-top:10px;font-size:11px;color:#64748b;font-style:italic;padding:7px 10px;background:#f8fafc;border-radius:8px;border-left:3px solid #cbd5e1">📝 বিশেষ নোট: ${rpl.note}</div>` : ''}
          </div>
        `;
      ...
      if (rplData) {
        try {
          ...
          setTimeout(async () => {
            ...
            // settlement অনুযায়ী বাকি/total আপডেট
            if (rplData.settleMethod === 'deduct' && latestSaleId) {
              // দেওয়া মাল নেই → মূল বিক্রয়ের total ও due থেকে ফেরত বিয়োগ
              const ls = shopSales[0]?.[1];
              if (ls) {
                const newDue   = Math.max(0, (ls.due||0) - rplData.retTotal);
                const newTotal = Math.max(0, (ls.total||0) - rplData.retTotal);
                await fbUpdate(`sales/${latestSaleId}`, {
                  due: newDue, total: newTotal,
                  hasReplacement: true, rplDeduct: rplData.retTotal,
                });
                toast(`✅ বিল থেকে ${bn(rplData.retTotal)} বিয়োগ হয়েছে`);
              }
            } else if (rplData.settleMethod === 'due' && rplData.diff !== 0 && latestSaleId) {
              const ls = shopSales[0]?.[1];
              if (ls) {
                await fbUpdate(`sales/${latestSaleId}`, {
                  due: (ls.due||0) + rplData.diff,
                  hasReplacement: true,
                });
              }
  ...
  function captureInlineRpl() {
    ...
    const diff = givTotal - retTotal;
    // দেওয়া মাল না থাকলে → বিক্রয় থেকে বিয়োগ হবে
    const noGivItems = _inGiv.length === 0;
    const settleMethod = noGivItems ? 'deduct' : (diff !== 0 ? ($('rplInSettle')?.value || 'due') : 'equal');
