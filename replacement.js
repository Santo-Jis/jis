...
<div class="rpl-wrap">
  ...
  window.rplSave = async function() {
    ...
    try {
      ...
      if (settleMethod === 'due' && diff !== 0) {
        ...
        toast(diff > 0 ? `✅ সংরক্ষিত — ${bn(diff)} দোকান আমাদের কাছে পাবে` : `✅ সংরক্ষিত — ${bn(Math.abs(diff))} আমরা দোকানের কাছে পাবো`);
  ...
  function renderList(filter) {
    ...
    el.innerHTML = items.map(r => {
      ...
      const diffText  = diff > 0 ? `+${bn(diff)} দোকান আমাদের কাছে পাবে`
                      : diff < 0 ? `-${bn(Math.abs(diff))} আমরা দোকানের কাছে পাবো`
                      : 'সমান মাল';
      const settleLabel = {
        'due':   '🏦 বাকিতে',
        'cash':  '💵 নগদে',
  ...
  window.rplPrint = function(id) {
    ...
    const diff = r.diff || 0;
    const diffText = diff > 0 ? `+${bn(diff)} (দোকান আমাদের কাছে পাবে)` : diff < 0 ? `-${bn(Math.abs(diff))} (আমরা দোকানের কাছে পাবো)` : 'সমান';
...
<script>window.onload=()=>window.print()</script>
    ...
    wrap.innerHTML = `
      ...
      ${recentItems.length ? recentItems.map(r => {
        ...
        return `<div class="rpl-item">
          ...
          <div class="rpl-item-foot">
            <div style="font-size:11px;font-weight:700;color:${diffColor}">${diff>0?'+'+bn(diff)+' দোকান আমাদের কাছে পাবে':diff<0?bn(Math.abs(diff))+' আমরা দোকানের কাছে পাবো':'সমান'}</div>
            <div style="display:flex;gap:6px">
  ...
  function srplCalc() {
    ...
    if (diff>0) {
      ...
      dlbl.textContent=`দোকান আমাদের কাছে পাবে`; dval.textContent=`+${bn(diff)}`;
      if(sb)sb.style.display='block';
    } else if (diff<0) {
      ...
      dlbl.textContent=`আমরা দোকানের কাছে পাবো`; dval.textContent=`-${bn(Math.abs(diff))}`;
      if(sb)sb.style.display='block';
    } else {
  ...
  window.srplSave = async function() {
    ...
    try {
      ...
      toast(diff>0?`✅ সংরক্ষিত — ${bn(diff)} বাকি যোগ`:diff<0?`✅ সংরক্ষিত — ${bn(Math.abs(diff))} আমরা দোকানের কাছে পাবো`:'✅ রিপ্লেসমেন্ট সংরক্ষিত');
      _srplRet=[]; _srplGiv=[];
      renderSalePageInline();
    } catch(e) { toast('ব্যর্থ: '+e.message,true); }
  };
