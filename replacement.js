...
<div class="rpl-wrap">
  ...
  <div class="rpl-card">
    ...
    <div class="rpl-mb" id="rplWorkerRow">
      ...
      <select class="rpl-sel" id="rplWorker"><option value="">-- কর্মী --</option></select>
    </div>
  ...
  function loadSelects() {
    ...
    if (window.CR === 'worker') {
      ...
      if (workerSel) workerSel.innerHTML = '<option value="">-- কর্মী --</option>' +
  ...
  window.rplSave = async function() {
    ...
    let uid, workerName;
    if (window.CR === 'worker') {
      uid = window.CU?.uid; workerName = window.CN;
    } else {
      uid = $('rplWorker')?.value;
      workerName = uid ? (users[uid]?.name || window.CN) : window.CN;
    }
    ...
    try {
      ...
      if (settleMethod === 'due' && diff !== 0) {
        ...
        toast(diff > 0 ? `✅ সংরক্ষিত — ${bn(diff)} বাকি যোগ হয়েছে` : `✅ সংরক্ষিত — ${bn(Math.abs(diff))} বাকি কমেছে`);
      ...
      // Reset
      _retItems = []; _givItems = [];
      ['rplShop','rplWorker'].forEach(id => { const el=$(id); if(el) el.value=''; });
  ...
  function renderList(filter) {
    ...
    el.innerHTML = items.map(r => {
      ...
      const diffText  = diff > 0 ? `+${bn(diff)} বাকি বেড়েছে`
                      : diff < 0 ? `-${bn(Math.abs(diff))} বাকি কমেছে`
                      : 'সমান মাল';
      const settleLabel = {
        'due':   '🏦 বাকিতে',
        'cash':  '💵 নগদে',
  ...
  window.rplPrint = function(id) {
    ...
    const diff = r.diff || 0;
    const diffText = diff > 0 ? `+${bn(diff)} (দোকান পাবে — বাকি)` : diff < 0 ? `-${bn(Math.abs(diff))} (কম নিয়েছে)` : 'সমান';
...
<script>window.onload=()=>window.print()</script>
    ...
    wrap.innerHTML = `
      ...
      <div class="form-card" style="margin-bottom:12px">
        ...
        <div style="margin-bottom:8px">
          <div class="rpl-lbl">কর্মী</div>
          <select id="srplWorker" style="background:var(--surface);border:1px solid var(--border);border-radius:9px;padding:9px 11px;color:var(--text);font-size:13px;font-family:inherit;width:100%">
            <option value="">-- কর্মী --</option>
            ${Object.entries(window.allUsers||{}).filter(([,u])=>u.role==='worker'||u.role==='manager').map(([id,u])=>`<option value="${id}">${u.name}</option>`).join('')}
          </select>
        </div>` : ''}
      ...
      ${recentItems.length ? recentItems.map(r => {
        ...
        return `<div class="rpl-item">
          ...
          <div class="rpl-item-foot">
            <div style="font-size:11px;font-weight:700;color:${diffColor}">${diff>0?'+'+bn(diff)+' বাকি বেড়েছে':diff<0?bn(Math.abs(diff))+' বাকি কমেছে':'সমান'}</div>
            <div style="display:flex;gap:6px">
  ...
  function srplRenderRows(type) {
    ...
    el.innerHTML = items.map((item,i)=>`
      <div style="display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:5px;margin-bottom:5px;align-items:center">
        ...
        <input type="number" value="${item.qty}" min="1" onchange="window.srplRowChange('${type}',${i},'qty',this)" style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 8px;color:var(--text);font-size:12px;width:100%">
        <input type="number" value="${item.price}" min="0" onchange="window.srplRowChange('${type}',${i},'price',this)" style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 8px;color:var(--text);font-size:12px;width:100%">
  ...
  function srplCalc() {
    ...
    if (diff>0) {
      ...
      dlbl.textContent=`দোকান ${bn(diff)} বেশি পাবে`; dval.textContent=`+${bn(diff)}`;
      if(sb)sb.style.display='block';
    } else if (diff<0) {
      ...
      dlbl.textContent=`দোকান ${bn(Math.abs(diff))} কম পাবে`; dval.textContent=`-${bn(Math.abs(diff))}`;
      if(sb)sb.style.display='block';
    } else {
  ...
  window.srplSave = async function() {
    ...
    if (window.CR!=='worker') {
      uid = $('srplWorker')?.value||window.CU?.uid;
      workerName = uid?(users[uid]?.name||window.CN):window.CN;
    }
    ...
    try {
      ...
      toast(diff>0?`✅ সংরক্ষিত — ${bn(diff)} বাকি যোগ`:diff<0?`✅ সংরক্ষিত — ${bn(Math.abs(diff))} বাকি কমেছে`:'✅ রিপ্লেসমেন্ট সংরক্ষিত');
      _srplRet=[]; _srplGiv=[];
      renderSalePageInline();
    } catch(e) { toast('ব্যর্থ: '+e.message,true); }
  };
  ...
  waitForApp(() => {
    ...
    // ── বিক্রয় form এ রিপ্লেসমেন্ট inline logic ──
    initInlineRpl();
  });
  ...
  function rplInRenderRows(type) {
    ...
    if (!el) return;
    const prods = window.allProducts || {};
    const color = type === 'ret' ? '#ef4444' : '#10b981';
    ...
    el.innerHTML = items.map((item, i) => `
      <div style="display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:5px;margin-bottom:5px;align-items:center">
        ...
        <input type="number" value="${item.qty}" min="1"
          onchange="window.rplInRowChange('${type}',${i},'qty',this)"
        ...
        <input type="number" value="${item.price}" min="0" placeholder="দাম"
          onchange="window.rplInRowChange('${type}',${i},'price',this)"
          style="background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:7px 8px;color:var(--text);font-size:12px;width:100%">
  ...
  function rplInCalc() {
    ...
    if (diff > 0) {
      dbox.style.cssText = 'display:flex;padding:9px 12px;border-radius:9px;margin-bottom:8px;font-size:12px;font-weight:700;align-items:center;justify-content:space-between;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#ef4444';
      dlbl.textContent = `দোকান ${bn(diff)} বেশি পাবে`;
      dval.textContent = `+${bn(diff)}`;
      if (sb) sb.style.display = 'block';
    } else if (diff < 0) {
      dbox.style.cssText = 'display:flex;padding:9px 12px;border-radius:9px;margin-bottom:8px;font-size:12px;font-weight:700;align-items:center;justify-content:space-between;background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);color:#10b981';
      dlbl.textContent = `দোকান ${bn(Math.abs(diff))} কম পাবে`;
