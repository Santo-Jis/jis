    ...
    try {
      ...
      if (settleMethod === 'due' && diff !== 0) {
        ...
        if (shopSales.length > 0) {
          ...
          const newDue = Math.max(0, (ls.due || 0) + diff);
          await fbUpdate(`sales/${lid}`, { due: newDue });
        }
...
<script>window.onload=()=>window.print()</script>
  ...
  window.srplSave = async function() {
    ...
    try {
      ...
      if (settleMethod==='due' && diff!==0) {
        ...
        if (shopSales.length>0) {
          const [lid,ls]=shopSales[0];
          await fbUpdate(`sales/${lid}`,{due:Math.max(0,(ls.due||0)+diff)});
        }
      }
  ...
  waitForApp(() => {
    ...
    window.generateInvoice = function(saleId) {
      ...
      // এই sale এর সাথে linked replacement আছে কিনা দেখি
      const rpl = Object.values(allReplacements).find(r => r.linkedSaleId === saleId);
      if (!rpl) return;

      // Invoice content এ রিপ্লেসমেন্ট section যোগ করি
      setTimeout(() => {
        const ic = document.getElementById('invoiceContent');
        if (!ic || ic.querySelector('#rpl-invoice-section')) return;
        ...
        const diffText = diff > 0 ? `আমরা দোকানের কাছে পাবো ${bn(diff)}`
                       : diff < 0 ? `দোকান আমাদের কাছে পাবে ${bn(Math.abs(diff))}`
                       : 'সমান মাল';
        ...
        rplSection.style.cssText = 'margin-top:12px;padding-top:12px;border-top:1px dashed #e2e8f0';
        rplSection.innerHTML = `
          <div style="font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">🔄 রিপ্লেসমেন্ট</div>

          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
            <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:8px">
              <div style="font-size:9px;font-weight:700;color:#dc2626;margin-bottom:5px">📥 ফেরত (কাস্টমার দিলো)</div>
              ${retItems.map(i=>`<div style="display:flex;justify-content:space-between;font-size:11px;padding:2px 0;border-bottom:1px solid rgba(0,0,0,.05)">
                <span>${i.prodName} ×${i.qty}</span><span style="font-weight:700">৳${i.qty*i.price}</span>
              </div>`).join('')}
              <div style="display:flex;justify-content:space-between;font-size:12px;font-weight:700;color:#dc2626;margin-top:5px;padding-top:4px;border-top:1px solid #fecaca">
                <span>মোট</span><span>${bn(rpl.retTotal||0)}</span>
              </div>
            </div>
            <div style="background:#f0fdf4;border:1px solid #a7f3d0;border-radius:8px;padding:8px">
              <div style="font-size:9px;font-weight:700;color:#059669;margin-bottom:5px">📤 দেওয়া (কর্মী দিলো)</div>
              ${givItems.map(i=>`<div style="display:flex;justify-content:space-between;font-size:11px;padding:2px 0;border-bottom:1px solid rgba(0,0,0,.05)">
                <span>${i.prodName} ×${i.qty}</span><span style="font-weight:700">৳${i.qty*i.price}</span>
              </div>`).join('')}
              <div style="display:flex;justify-content:space-between;font-size:12px;font-weight:700;color:#059669;margin-top:5px;padding-top:4px;border-top:1px solid #a7f3d0">
                <span>মোট</span><span>${bn(rpl.givTotal||0)}</span>
              </div>
            </div>
          </div>

          ${diff !== 0 ? `<div style="padding:8px 10px;border-radius:8px;background:${diff>0?'#f0fdf4':'#fef2f2'};border:1px solid ${diff>0?'#a7f3d0':'#fecaca'};display:flex;justify-content:space-between;font-size:12px;font-weight:700;color:${diffColor}">
            <span>${diffText.split(' ').slice(0,4).join(' ')}</span>
            <span>${bn(Math.abs(diff))}</span>
          </div>` : `<div style="text-align:center;font-size:11px;color:#64748b">⚖️ সমান মাল — কোনো পার্থক্য নেই</div>`}

          ${rpl.note ? `<div style="font-size:10px;color:#64748b;margin-top:6px;font-style:italic">📝 ${rpl.note}</div>` : ''}
        `;

        // Invoice এর শেষে যোগ করি (close button এর আগে)
        const closeArea = ic.querySelector('[onclick*="close"]') || ic.lastElementChild;
        ic.insertBefore(rplSection, closeArea);
      }, 100);
    };
  });
