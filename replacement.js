...
<div class="rpl-wrap">
  ...
  window.rplSave = async function() {
    ...
    try {
      ...
      // ✅ Worker dashboard refresh — due ও replacement update হয়েছে
      // ১ সেকেন্ড ও ৩ সেকেন্ড দুইবার refresh — Firebase propagation নিশ্চিত করতে
      setTimeout(() => { if(typeof window._wdSetPeriod==='function') window._wdSetPeriod(window._wdPeriod||'1'); }, 1000);
      setTimeout(() => { if(typeof window._wdSetPeriod==='function') window._wdSetPeriod(window._wdPeriod||'1'); }, 3000);
...
<script>window.onload=()=>window.print()</script>
  ...
  function startListener() {
    fbOnVal('replacements', snap => {
      ...
      window.allReplacements = allReplacements; // ✅ worker-dashboard এর জন্য
      const pg = $('page-replace');
      if (pg && pg.style.display !== 'none') renderList(_filter);
      renderSalePageInline();
      // ✅ Worker dashboard refresh — replacement data বদলালে
      if (typeof window._wdSetPeriod === 'function') {
        window._wdSetPeriod(window._wdPeriod || '1');
      }
    });
  }
            ...
            <div style="position:relative;padding-left:22px">
              ...
              <div style="position:relative">
                ...
                <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:10px">
                  ...
                  <!-- ফলাফল -->
                  <div style="padding:12px;border-radius:10px;text-align:center;background:${netDue>0?'#eff6ff':netDue<0?'#fffbeb':'#f0fdf4'};border:1.5px solid ${netDue>0?'#93c5fd':netDue<0?'#fde68a':'#86efac'}">
                    ${netDue > 0 ? `
