...
(function () {
  ...
  style.textContent = `
    ...
    /* ══ CHAT SYSTEM v2 ══ */
    .chat-screen{display:flex;flex-direction:column;height:calc(100vh - 130px);background:var(--bg);}
    /* Conversation List */
    ...
    /* Chat Room */
    .chat-room-wrap{display:flex;flex-direction:column;height:calc(100vh - 130px);}
  ...
  function patchShowPage() {
    ...
    window.showPage = function (id) {
      ...
      if (id === 'chat')  { setTimeout(ftRenderConvList, 80); }
  ...
  function ftRenderConvList() {
    ...
    rooms.push({
      ...
      sub: 'সকল কর্মী',  av: 'conv-av-public'
    });
    ...
    if (window.CR !== 'worker') {
      rooms.push({
        ...
        sub: 'Admin ও Manager', av: 'conv-av-group'
      });
    }
    ...
    // ৪. DM — অন্য সব ব্যবহারকারী
    Object.entries(users).forEach(([uid, u]) => {
      if (uid === myUid) return;
      const dmId = [myUid, uid].sort().join('_');
      rooms.push({
        id: 'dm_' + dmId, type: 'dm', icon: (u.name||'?')[0].toUpperCase(),
        name: u.name || u.email, sub: u.role === 'worker' ? 'কর্মী' : u.role === 'manager' ? 'ম্যানেজার' : 'অ্যাডমিন',
    ...
    el.innerHTML = rooms.map(r => {
      ...
      const preview = last ? (last.text || (last.imageUrl ? '📷 ছবি' : last.fileUrl ? '📎 ফাইল' : '')) : 'কথোপকথন শুরু করুন';
      const time = last ? new Date(last.ts).toLocaleTimeString('bn-BD',{hour:'2-digit',minute:'2-digit'}) : '';
      const isOnline = r.type==='dm' && r.targetUid && _onlineUsers?.[r.targetUid];

      return `<div class="conv-item" onclick="ftOpenRoom(${JSON.stringify(r).replace(/"/g,'&quot;')})">
        <div class="conv-avatar ${r.av}" style="${r.type==='dm'?'font-size:16px;':''}">
          ${r.icon}
  ...
  window.ftOpenRoom = function(r) {
    ...
    // Header আপডেট
    const av = $('ft_roomAvatar'); if(av) av.textContent = r.icon;
    const title = $('ft_roomTitle'); if(title) title.textContent = r.name;
    const sub = $('ft_roomSub');
    if (sub) {
      if (r.type==='public') sub.textContent = r.sub;
      else if (r.type==='group') sub.textContent = r.sub;
      else sub.textContent = r.sub;
    }
