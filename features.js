...
(function () {
  ...
  style.textContent = `
    ...
    /* Chat bubbles */
    .chat-wrap{display:flex;flex-direction:column;height:calc(100vh - 165px)}
    .chat-msgs{flex:1;overflow-y:auto;padding:10px;display:flex;flex-direction:column;gap:2px}
    .chat-bubble-me{align-self:flex-end;background:linear-gradient(135deg,var(--accent),#d97706);color:#1a1200;padding:9px 13px;border-radius:14px 14px 2px 14px;font-size:13px;max-width:74%;margin-bottom:8px;line-height:1.5}
    .chat-bubble-other{align-self:flex-start;background:var(--card);border:1px solid var(--border);color:var(--text);padding:9px 13px;border-radius:2px 14px 14px 14px;font-size:13px;max-width:74%;margin-bottom:8px;line-height:1.5}
    .chat-meta-me{font-size:10px;color:rgba(26,18,0,.6);margin-top:3px;text-align:right}
    .chat-meta-other{font-size:10px;color:var(--muted);margin-top:3px}
    .chat-input-row{display:flex;gap:8px;padding:10px 12px;background:var(--surface);border-top:1px solid var(--border)}
  ...
  function injectHTML() {
    ...
    if (!document.getElementById('page-chat')) {
      ...
      chatPage.innerHTML = `
        <div class="chat-wrap" style="margin-top:8px">
          <div style="display:flex;gap:5px;padding:0 0 8px;overflow-x:auto;scrollbar-width:none">
            <button class="fb active chat-room-btn" onclick="ftSwitchRoom('general',this)">💬 সাধারণ</button>
            <button class="fb chat-room-btn" onclick="ftSwitchRoom('sales',this)">🛍 বিক্রয়</button>
            <button class="fb chat-room-btn" onclick="ftSwitchRoom('alerts',this)">🚨 অ্যালার্ট</button>
            <button class="fb chat-room-btn" onclick="ftSwitchRoom('management',this)" id="ftMgmtRoomBtn">👔 ম্যানেজমেন্ট</button>
          </div>
          <div id="ft_chatMsgs" class="chat-msgs" style="background:var(--surface);border-radius:12px 12px 0 0;border:1px solid var(--border);border-bottom:none"></div>
          <div class="chat-input-row" style="border-radius:0 0 12px 12px;border:1px solid var(--border)">
            ...
            <button class="ft-btn ft-btn-b" onclick="ftSendFile()" style="padding:7px 10px">📎</button>
  ...
  function patchShowPage() {
    ...
    if (!origShow || window._ftShowPatched) return;
    window._ftShowPatched = true;
    window.showPage = function (id) {
      origShow.call(this, id);
      if (id === 'tasks') { setTimeout(ftRenderTasks, 80); ftLoadWorkerSelects(); }
      if (id === 'chat')  { setTimeout(ftRenderChat, 80); }
      if (id === 'order') { setTimeout(() => { ftRenderOrders('all'); ftLoadOrderSelects(); }, 80); }
    };
  }
  ...
  window.ftUpdateTask = async function (taskId, status) {
    ...
    await fbUpdate(fbRef(db, 'tasks/' + taskId), {
      status,
      updatedAt: Date.now(),
      updatedBy: window.CU.uid
    });
    toast({ done: '✅ সম্পন্ন!', in_progress: '🔄 শুরু হয়েছে', cancelled: '❌ বাতিল' }[status] || 'আপডেট');
  };
  ...
  window.ftSendMsg = async function () {
    const input = $('ft_chatInput'); if (!input) return;
    const text = input.value.trim(); if (!text) return;
    input.value = '';

    const db = DB(); if (!db) return;
    await fbPush(fbRef(db, 'chats/' + currentChatRoom), {
      text,
      uid: window.CU.uid,
      name: window.CN,
      role: window.CR,
  ...
  window.ftSwitchRoom = function (room, btn) {
  ...
  function ftRenderChat() {
    const el = $('ft_chatMsgs'); if (!el) return;
    const msgs = Object.values(allChats[currentChatRoom] || {})
      .sort((a, b) => a.ts - b.ts)
      .slice(-60);

    if (!msgs.length) {
      el.innerHTML = `<div style="text-align:center;padding:30px;color:var(--muted);font-size:13px">💬 কথোপকথন শুরু করুন</div>`;
      return;
    }
    ...
    el.innerHTML = msgs.map(m => {
      ...
      const avatar = `<div style="width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--blue));display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0">${(m.name || '?').charAt(0)}</div>`;
      const bubble = `
        ${m.text ? `<div>${m.text}</div>` : ''}
        ${m.imageUrl ? `<img src="${m.imageUrl}" style="width:100%;border-radius:8px;margin-top:6px;cursor:pointer;max-height:160px;object-fit:cover" onclick="window.open('${m.imageUrl}','_blank')" loading="lazy">` : ''}
        ${m.fileUrl ? `<a href="${m.fileUrl}" target="_blank" style="color:inherit;font-size:11px">📎 ${m.fileName || 'ফাইল'}</a>` : ''}`;

      return isMe
        ? `<div style="display:flex;flex-direction:column;align-items:flex-end">
             <div class="chat-bubble-me">${bubble}</div>
             <div class="chat-meta-me">${time}</div>
           </div>`
        : `<div style="display:flex;gap:7px;align-items:flex-end">
             ${avatar}
             <div>
               <div style="font-size:10px;color:var(--muted);margin-bottom:2px">${m.name} <span class="role-badge role-${m.role}" style="font-size:8px">${m.role}</span></div>
  ...
  window.ftUpdateOrder = async function (ordId, status) {
    ...
    toast({ approved: '✅ অর্ডার অনুমোদিত', delivered: '🚚 ডেলিভারি সম্পন্ন', cancelled: '❌ বাতিল' }[status]);
  ...
  function startListeners() {
    ...
    fbOnValue(fbRef(db, 'chats'), snap => {
      allChats = snap.val() || {};
      if (document.querySelector('#page-chat.active')) ftRenderChat();
    });
