...
.nt-sum-val{color:#f0f6ff;font-weight:700;}
  ...
  async function shouldShowCheckin() {
    ...
    if (window.CR !== 'worker') return false;
    const today = now.toISOString().split('T')[0];
    // ✅ Firebase থেকে directly check করি — reopen এ accurate হবে
    try {
      const db = window._db; if(!db) return false;
      const { get, ref, query, orderByChild, equalTo } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js');
      const snap = await get(query(ref(db,'attendance'), orderByChild('uid'), equalTo(window.CU?.uid)));
      if(snap.exists()) {
        const entries = Object.values(snap.val());
        const todayDone = entries.find(a => a.date === today && a.checkIn);
        if(todayDone) return false; // আজকে আগেই check-in হয়েছে
      }
    } catch(e) {
      // Firebase check fail হলে local cache দেখি
  ...
  async function showCheckinPopup() {
    if (!(await shouldShowCheckin())) return;
    ...
    el.innerHTML = `<div id="ntCheckinBox">
      ...
      <div id="ntStep2" style="display:none;">
        ...
        <div class="nt-btn-row">
          <button class="nt-btn-primary" id="ntSelfieNext" onclick="window._ntGoStep3()" style="display:none;width:100%;">পরবর্তী →</button>
          <div id="ntSelfieWait" style="width:100%;padding:12px;text-align:center;font-size:13px;color:#8ba3c4;">📷 সেলফি তুলুন</div>
        </div>
      </div>
      <div id="ntStep3" style="display:none;">
        ...
        <div class="nt-btn-row">
          <button class="nt-btn-primary" id="ntGpsNext" onclick="window._ntGoSuccess()" style="display:none;width:100%;">সম্পন্ন ✓</button>
          <div id="ntGpsWait" style="width:100%;padding:12px;text-align:center;font-size:13px;color:#8ba3c4;">📡 GPS নেওয়া হচ্ছে...</div>
        </div>
      </div>
  ...
  window._ntSelfieSelected = function(input) {
    ...
    reader.onload = e => {
      ...
      const sw=document.getElementById('ntSelfieWait'); if(sw) sw.style.display='none';
    };
      ...
      if(p >= 100) {
        ...
        if(navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(pos => {
            ...
            const gw=document.getElementById('ntGpsWait'); if(gw) gw.style.display='none';
          }, () => {
            document.getElementById('ntGpsStatus').textContent = '⚠️ GPS পাওয়া যায়নি — তবুও চেক-ইন হবে';
            document.getElementById('ntGpsNext').style.display = 'block';
            const gw=document.getElementById('ntGpsWait'); if(gw) gw.style.display='none';
          }, {timeout:12000, enableHighAccuracy:true});
        } else {
          document.getElementById('ntGpsStatus').textContent = '⚠️ GPS সাপোর্ট নেই';
          document.getElementById('ntGpsNext').style.display = 'block';
          const gw=document.getElementById('ntGpsWait'); if(gw) gw.style.display='none';
        }
      }
  ...
  waitForApp(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(async () => { if(typeof window.showCheckinPopup==='function') await window.showCheckinPopup(); }, 1000);
