...
(function () {
  ...
  function waitForApp(cb, retries = 0) {
    ...
    if (retries > 50) { console.warn('waitForApp: timeout after 20s'); return; }
  ...
  window.ftOpenRoom = function(r) {
    ...
    // ✅ FIX: visit time সেভ + Firebase এ read mark করি
    if (!window._lastChatVisit) window._lastChatVisit = {};
    window._lastChatVisit[r.id] = Date.now();

    // Firebase এ অন্যদের message read:true করি
    _markRoomAsRead(r.id);

    // badge তাৎক্ষণিক clear করি
  ...
  // ── চ্যাট রুম রেন্ডার
  // ✅ FIX: Firebase এ room এর সব unread message read:true করি
  // ✅ সব room একসাথে read mark — chat page open থাকলে
    ...
    try {
      ...
      Object.entries(allChats).forEach(([roomId, msgs]) => {
        Object.entries(msgs||{}).forEach(([msgKey, msg]) => {
          if (msg && msg.uid !== myUid && !msg.read) {
            updates['chats/' + roomId + '/' + msgKey + '/read'] = true;
          }
        });
      });
  ...
  async function _markRoomAsRead(roomId) {
    try {
      ...
      const updates = {};
      Object.entries(msgs).forEach(([msgKey, msg]) => {
        // নিজের message নয়, এবং read হয়নি এমন message
        if (msg && msg.uid !== myUid && !msg.read) {
          updates['chats/' + roomId + '/' + msgKey + '/read'] = true;
        }
      });
  ...
  function startListeners() {
    ...
    fbOnValue(fbRef(db, 'chats'), snap => {
      ...
      if (_currentRoom) {
        ftRenderChatRoom();
        // ✅ room open থাকলে নতুন message ও read mark করি
        _markRoomAsRead(_currentRoom.id);
      } else if (document.querySelector('#page-chat.active')) {
        ftRenderConvList();
        // ✅ chat page খোলা থাকলে সব room read mark করি
        _markAllRoomsAsRead();
      }
      // notification badge
      ftUpdateChatBadge();
  ...
  function ftUpdateChatBadge() {
    ...
    Object.entries(allChats).forEach(([roomId, msgs]) => {
      ...
      // ✅ FIX: Firebase এ read:true mark হয়, তাই শুধু !m.read check করি
      const unread = msgList.filter(m => m.uid !== myUid && !m.read).length;
    ...
    // ✅ Drawer চ্যাট item badge — কোন room এ unread আছে সেটা drawer এ দেখাবে
    const drawerBadge = document.getElementById('drawerChatBadge');
    if (drawerBadge) {
      if (totalUnread > 0) {
        drawerBadge.textContent = totalUnread > 9 ? '9+' : String(totalUnread);
  ...
  waitForApp(() => {
    ...
    setTimeout(() => {
      ...
      window.ftLoadOrderSelects = ftLoadOrderSelects;
      window.ftLoadWorkerSelects = ftLoadWorkerSelects;
      // ✅ badge + read mark functions expose
      window._ftMarkAllRead = () => _markAllRoomsAsRead();
