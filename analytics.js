...
async function runAIAnalysis(forceRefresh = false) {
  ...
  try {
    ...
    if(!apiKey){
      el.innerHTML = '<div style="padding:20px;text-align:center;color:var(--red)"><div style="font-size:24px">🔑</div><div style="font-size:13px;margin-top:8px">AI Config এ Anthropic API Key দিন</div><div style="font-size:11px;color:var(--muted);margin-top:4px">Admin → ⚙️ → 🤖 Claude AI কনফিগারেশন</div></div>';
      return;
    }
...
function runAnomalyDetection() {
  ...
  // 5. Zero sales day
  const data_ = buildAnalyticsData();
  const recentZero = data_.dailyTrend.slice(-7).filter(d => d.revenue === 0);
