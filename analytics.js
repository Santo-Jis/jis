...
<div class="section">
  ...
  <table>
    <tr><th>কর্মী</th><th>বিক্রয়</th>${window.CR==='admin'?'<th>লাভ</th>':''}<th>টার্গেট</th><th>অর্জন</th><th>উপস্থিতি</th><th>দেরি</th></tr>
    ${workers.map(w => `<tr>
      <td><b>${w.name}</b></td>
      <td>৳${bnNum(w.revenue)}</td>
      ${window.CR==='admin'?`<td>৳${bnNum(w.profit)}</td>`:''}
...
<div class="section">
  <h2>📦 শীর্ষ পণ্য</h2>
  <table>
    <tr><th>পণ্য</th><th>বিক্রয় (পিস)</th><th>রাজস্ব</th>${window.CR==='admin'?'<th>লাভ</th>':''}</tr>
    ${products.map(([name,s]) => `<tr>
      <td>${name}</td><td>${bnNum(s.qty)} পিস</td>
      <td>৳${bnNum(s.revenue)}</td>${window.CR==='admin'?`<td>৳${bnNum(s.profit)}</td>`:''}
    </tr>`).join('')}
  </table>
</div>` : ''}
...
async function getAIKey() {
  // ✅ window-এ key রাখি না — প্রতিবার Firebase থেকে নিই
  try {
    const db = window._firebaseDB || window._db;
    if(db) {
      const { ref, get } = await import('https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js');
      ...
      if(snap.exists()) {
        ...
        if(key) return key; // ✅ return করি, window-এ রাখি না
      }
    }
  } catch(e) { /* silent */ }
  return ''; // key না পেলে খালি
}
...
function showAINotification(title, message, type='info') {
  ...
  if(badge){badge.textContent=aiNotifications.filter(n=>!n.read).length;badge.style.display='flex';}
}
...
// ── ১. ৭ দিনের Line Chart (বিক্রয় + খরচ)
window.drawDashLineChart = function() {
  const r = setupCanvas('dashLineChart', 110); if (!r) return;
  const { ctx, w, h } = r;
  const days = [], labels = [];
  for (let i = 6; i >= 0; i--) {
    const d = new Date(); d.setDate(d.getDate() - i);
  ...
  const sales = Object.values(window.allSales   || {});
  const exps  = Object.values(window.allExpenses || {});
  const saleV = days.map(d => sales.filter(s => s.date===d).reduce((a,b) => a+(b.total||0),  0));
  const expV  = days.map(d => exps.filter(e  => e.date===d).reduce((a,b) => a+(b.amount||0), 0));
  const maxV  = Math.max(...saleV, ...expV, 1);
  ...
  [0.25,0.5,0.75,1].forEach(t=>{const y=padT+ch*(1-t);ctx.beginPath();ctx.moveTo(padL,y);ctx.lineTo(padL+cw,y);ctx.stroke();});
  // Fill
  const grad=ctx.createLinearGradient(0,padT,0,padT+ch);
  grad.addColorStop(0,'rgba(59,130,246,0.25)');grad.addColorStop(1,'rgba(59,130,246,0)');
  ctx.beginPath();
  saleV.forEach((v,i)=>{const x=padL+i*step,y=padT+ch*(1-v/maxV);i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});
  ...
  // Expense line
  ctx.beginPath();
  expV.forEach((v,i)=>{const x=padL+i*step,y=padT+ch*(1-v/maxV);i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});
  ctx.strokeStyle='#EF4444';ctx.lineWidth=1.5;ctx.setLineDash([4,3]);ctx.stroke();ctx.setLineDash([]);
  // Labels
  ctx.fillStyle='rgba(148,163,184,0.9)';ctx.font='9px Hind Siliguri,sans-serif';ctx.textAlign='center';
  labels.forEach((l,i)=>ctx.fillText(l,padL+i*step,h-4));
  // Last dot
  const lx=padL+(days.length-1)*step, ly=padT+ch*(1-saleV[saleV.length-1]/maxV);
  ctx.beginPath();ctx.arc(lx,ly,4,0,Math.PI*2);ctx.fillStyle='white';ctx.fill();
  ctx.strokeStyle='#3B82F6';ctx.lineWidth=2;ctx.stroke();
};

// ── ২. কর্মী Bar Chart (এই মাস)
window.drawDashWorkerChart = function() {
  const r = setupCanvas('dashWorkerChart', 120); if (!r) return;
  const { ctx, w, h } = r;
  const now = new Date();
  ...
  const sales = Object.values(window.allSales||{}).filter(s=>{
    const d=new Date(s.date);
    const inMonth = d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();
    if (!inMonth) return false;
    if (allowedUids) return allowedUids.has(s.uid);
    return true;
  });
  const wm={};
  sales.forEach(s=>{if(!s.workerName)return;wm[s.workerName]=(wm[s.workerName]||0)+(s.total||0);});
  const sorted=Object.entries(wm).sort((a,b)=>b[1]-a[1]).slice(0,5);
  if(!sorted.length)return;
  const maxV=sorted[0][1]||1, n=sorted.length;
  const rowH=h/n, labelW=70, padR=8, padL=6, padT=4;
  const barAreaW=w-padL-padR-labelW;
  ctx.clearRect(0,0,w,h);
  const colors=['#3B82F6','#10B981','#F59E0B','#8B5CF6','#EF4444'];
  sorted.forEach(([name,val],i)=>{
    const y=padT+i*rowH, barW=barAreaW*val/maxV;
    ctx.fillStyle='rgba(148,163,184,0.85)';ctx.font='10px Hind Siliguri,sans-serif';ctx.textAlign='left';
    ctx.fillText(name.length>7?name.slice(0,6)+'…':name, padL, y+rowH*0.65);
    ctx.fillStyle='rgba(30,42,64,0.8)';ctx.beginPath();
    if(ctx.roundRect)ctx.roundRect(padL+labelW,y+rowH*.2,barAreaW,rowH*.55,3);
    else ctx.rect(padL+labelW,y+rowH*.2,barAreaW,rowH*.55);ctx.fill();
    if(barW>0){
      const g=ctx.createLinearGradient(padL+labelW,0,padL+labelW+barW,0);
      g.addColorStop(0,colors[i]+'cc');g.addColorStop(1,colors[i]);
      ctx.fillStyle=g;ctx.beginPath();
      if(ctx.roundRect)ctx.roundRect(padL+labelW,y+rowH*.2,barW,rowH*.55,3);
      else ctx.rect(padL+labelW,y+rowH*.2,barW,rowH*.55);ctx.fill();
    }
    const bnV='৳'+(val>=1000?Math.round(val/1000)+'K':Math.round(val));
    ctx.fillStyle='rgba(255,255,255,0.85)';ctx.font='9px Sora,sans-serif';ctx.textAlign='right';
    ctx.fillText(bnV,w-padR,y+rowH*0.65);
  });
};

// ── ৩. Worker Progress Ring
window.drawWorkerProgressRing = function() {
  const el=document.getElementById('dashWorkerRing');if(!el)return;
  const sal=window.allSalaries?.[window.CU?.uid];
  const target=sal?.monthlyTarget||0;
  if(!target){el.style.display='none';return;}
  const now=new Date();
  const mySales=Object.values(window.allSales||{}).filter(s=>{
    const d=new Date(s.date);return s.uid===window.CU?.uid&&d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();
  });
  ...
  const todayTotal=Object.values(window.allSales||{})
    .filter(s=>s.uid===window.CU?.uid&&s.date===new Date().toISOString().split('T')[0])
    .reduce((a,b)=>a+(b.total||0),0);
  el.style.display='block';
  el.innerHTML=`<div style="background:var(--card);border-radius:var(--r);padding:14px;border:1px solid var(--border-l);display:flex;align-items:center;gap:16px;margin-bottom:12px;">
    <div style="position:relative;width:76px;height:76px;flex-shrink:0;">
      <svg width="76" height="76" viewBox="0 0 76 76">
        <circle cx="38" cy="38" r="30" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="10"/>
        <circle cx="38" cy="38" r="30" fill="none" stroke="${color}" stroke-width="10"
          stroke-dasharray="${2*Math.PI*30}" stroke-dashoffset="${2*Math.PI*30*(1-pct/100)}"
          stroke-linecap="round" transform="rotate(-90 38 38)"/>
        <text x="38" y="34" text-anchor="middle" fill="white" style="font-size:13px;font-weight:700;font-family:'Sora',sans-serif">${Math.round(pct)}%</text>
        <text x="38" y="48" text-anchor="middle" fill="rgba(148,163,184,0.9)" style="font-size:8px;font-family:'Hind Siliguri',sans-serif">টার্গেট</text>
      </svg>
    </div>
    <div style="flex:1;min-width:0;">
      <div style="font-size:12px;font-weight:700;color:var(--text);margin-bottom:6px;">🎯 মাসিক টার্গেট</div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:4px;">অর্জিত: <span style="color:${color};font-weight:700;">৳${Math.round(achieved).toLocaleString('bn-BD')}</span> / ৳${Math.round(target).toLocaleString('bn-BD')}</div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:8px;">আজকের: <span style="color:var(--blue);font-weight:700;">৳${Math.round(todayTotal).toLocaleString('bn-BD')}</span></div>
...
// ✅ Dashboard charts একসাথে
window.drawDashboardCharts = function() {
  const role=window.CR;
  const lineWrap=document.getElementById('dashLineChartWrap');
  if(lineWrap){
    if(role==='admin'||role==='manager'){lineWrap.style.display='block';setTimeout(window.drawDashLineChart,50);}
    else lineWrap.style.display='none';
  }
  const workerWrap=document.getElementById('dashWorkerChartWrap');
  if(workerWrap){
    if(role==='admin'||role==='manager'){workerWrap.style.display='block';setTimeout(window.drawDashWorkerChart,50);}
    else workerWrap.style.display='none';
  }
  if(role==='worker') setTimeout(window.drawWorkerProgressRing,50);
};
