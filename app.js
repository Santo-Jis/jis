...
async function uploadDocToDrive(file){
  try{
    ...
    else{showToast('আপলোড ব্যর্থ',true);return null;}
  }catch(e){showToast('আপলোড ব্যর্থ: '+e.message,true);return null;}
}

async function uploadFile(file,storagePath='uploads'){
  const imgTypes=['image/jpeg','image/png','image/gif','image/webp'];
  return imgTypes.includes(file.type)?await uploadImageToFirebase(file,storagePath):await uploadDocToDrive(file);
}
...
window.uploadProductPhoto=async(input,callback)=>{
  const file=input.files[0];if(!file)return;
  showToast('পণ্যের ছবি আপলোড হচ্ছে...');
...
function renderProfile(){
  ...
  if(uData?.photoURL){
    ...
    if(img){img.src=uData.photoURL;img.style.display='block';}
