...
let allSales={},allExpenses={},allPaymentLogs={},allProducts={},allUsers={},allAllowances={},allCustomers={},allRoutes={},allRouteRequests={},allStock={},allStockAssign={},allAttendance={},allLeaves={},allSalaries={},allCommConfig={},allNotices={},allTeams={},allSMSConfig={};
...
function initApp(){
  ...
  onValue(ref(db,'sales'),s=>{allSales=s.val()||{};syncGlobals();refreshDash();renderSaleList();renderDue();if(CR==='admin')renderReport();renderProfile();
...
function getFilteredSales(){
  ...
  return Object.values(allSales).filter(s=>{
    ...
    if(filterMode==='week'){const st=new Date(now);st.setDate(now.getDate()-6);st.setHours(0,0,0,0);return new Date(s.date)>=st;}
...
function refreshDash(){
  ...
  const totalDue    = Object.values(allSales).reduce((s,i)=>s+(CR==='worker'&&i.uid!==CU.uid?0:(i.due||0)),0);
  ...
  const ydSales = Object.values(allSales).filter(s=>s.date===ydStr&&(CR!=='worker'||s.uid===CU.uid));
...
function renderMonthProgress(){
  ...
  const mDue=Object.values(allSales).filter(s=>s.due>0).reduce((a,b)=>a+(b.due||0),0);
...
function renderCustomers(){
  ...
  el.innerHTML=list.length?list.map(([id,c])=>{
    ...
    const lastOrder=Object.values(allSales).filter(s=>s.shopId===id).sort((a,b)=>b.ts-a.ts)[0];
    const custDue=Object.values(allSales).filter(s=>s.shopId===id&&s.due>0).reduce((a,s)=>a+s.due,0);
    const totalSale=Object.values(allSales).filter(s=>s.shopId===id).reduce((a,s)=>a+(s.total||0),0);
...
function saleCard(s){
  ...
  // ✅ sale id বের করি
  const saleId=Object.keys(allSales).find(k=>allSales[k]===s)||'';
  ...
  return`<div class="ec"><div class="ei">
    <div class="shop">${s.shop} ${invTag}</div>
    <div class="prod">🛍 ${s.product} × ${s.qty} পিস ${s.disc>0?`· ছাড়: ${s.disc}%`:''}</div>
    <div class="dt">📅 ${fmtDate(s.date)} · <span class="wtag">${s.workerName||''}</span></div>
    ${statusTag}
    ${invoiceBtn}
  </div>
  <div class="ea">
...
function showPaymentReceipt(shop, amount, saleIds) {
  ...
  <div style="font-family:'Hind Siliguri',Arial,sans-serif;padding:16px;color:#1a202c;max-width:400px;">
    ...
    <div style="text-align:center;font-size:10px;color:#94a3b8;padding-top:8px;
      ...
      NovaTEch BD · বাকি আদায়ের নিশ্চিতকরণ 🙏
    </div>
  </div>`;
  openMo('invoiceMo');
}

// ✅ কাস্টমার card থেকে বিক্রয় page-এ যাওয়া
...
function renderSalary(){
  ...
  if(CR==='worker'){
    ...
    if(det&&sal){
      ...
      const mySales=Object.values(allSales).filter(s=>{const d=new Date(s.date);return s.uid===CU.uid&&d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();});
      const dailyMap={};
      mySales.filter(s=>isCommEligible(s)).forEach(s=>{dailyMap[s.date]=(dailyMap[s.date]||0)+s.total;});
  ...
  if(CR==='worker'){
    ...
    const now=new Date();
    const myS=Object.values(allSales).filter(s=>{const d=new Date(s.date);return s.uid===CU.uid&&d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()&&isCommEligible(s);});
    const dailyMap=buildDailyCommMap(myS);
    const comm=Object.values(dailyMap).reduce((a,v)=>a+calcCommission(v,allCommConfig||getDefaultSlabs()),0);
  ...
  Object.values(allSales).forEach(s=>{
    ...
    if(d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()&&isCommEligible(s)){
...
function renderProfile(){
  ...
  const myDueTotal=Object.values(allSales).filter(s=>s.uid===CU.uid&&s.due>0).reduce((a,s)=>a+s.due,0);
...
window.generateInvoice=(saleId)=>{
  ...
  // ✅ এই দোকানের payment history — বিক্রয়ের পরে যা পরিশোধ হয়েছে
  const saleDate = s.ts || 0;
  const payLogs = Object.values(allPaymentLogs||{})
    .filter(p => p.shop === s.shop && (p.ts||0) >= saleDate)
    .sort((a,b) => (a.ts||0) - (b.ts||0));

  // ✅ বাকি রাখার তথ্য
  const initialDue = s.payStatus==='due' ? s.total :
                     s.payStatus==='partial' ? cashPaid > 0 ? Math.round(s.total - cashPaid) : s.due : 0;
  const hasHistory = payLogs.length > 0 || initialDue > 0;
  ...
  const payHistoryHTML = hasHistory ? `
    <div style="margin-bottom:10px;">
      ...
      <!-- বিক্রয়ের সময় প্রাথমিক পেমেন্ট -->
      <div style="display:flex;gap:10px;margin-bottom:6px;">
        <div style="display:flex;flex-direction:column;align-items:center;">
          ...
          ${payLogs.length||s.due>0?`<div style="width:2px;flex:1;background:#e2e8f0;min-height:16px;"></div>`:''}
        </div>
        <div style="flex:1;padding-bottom:8px;">
          <div style="font-size:12px;font-weight:600;color:#1a202c;">বিক্রয় সম্পন্ন</div>
          <div style="font-size:11px;color:#64748b;">
            ${s.ts?new Date(s.ts).toLocaleString('bn-BD',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}):''}
          </div>
          <div style="font-size:11px;margin-top:2px;">
            ${s.payStatus==='paid'
              ? `<span style="color:#059669;font-weight:600;">মোট ${bn(s.total)} নগদ পরিশোধ ✅</span>`
              : s.payStatus==='partial'
              ? `<span style="color:#059669;">নগদ ${bn(cashPaid)} প্রদান</span>
                 <span style="color:#ef4444;margin-left:8px;">বাকি ${bn(initialDue)} রাখা হয়েছে ⚠️</span>`
              : `<span style="color:#ef4444;">মোট ${bn(s.total)} বাকি রাখা হয়েছে ⚠️</span>`
            }
          </div>
        </div>
      </div>
      ...
      ${payLogs.map((p, i) => {
        ...
        const payTime = p.ts ? new Date(p.ts).toLocaleString('bn-BD',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'}) : p.date||'';
        return `<div style="display:flex;gap:10px;margin-bottom:6px;">
          <div style="display:flex;flex-direction:column;align-items:center;">
            ...
            ${!isLast?`<div style="width:2px;flex:1;background:#e2e8f0;min-height:16px;"></div>`:''}
          </div>
          <div style="flex:1;padding-bottom:6px;">
            <div style="font-size:12px;font-weight:700;color:#059669;">💰 বাকি পরিশোধ</div>
            <div style="font-size:11px;color:#64748b;">${payTime}</div>
            <div style="font-size:12px;font-weight:600;color:#1a202c;margin-top:2px;">
              ${bn(p.amount)} আদায় করেছেন ${p.collectedBy||'–'}
            </div>
          </div>
        </div>`;
      }).join('')}
      ...
      <!-- বর্তমান অবস্থা -->
      ${s.due > 0 ? `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;
        padding:8px 10px;font-size:12px;font-weight:700;color:#dc2626;margin-top:4px;">
        ⚠️ এখনো বাকি: ${bn(s.due)}
      </div>` : payLogs.length > 0 ? `<div style="background:#ecfdf5;border:1px solid #a7f3d0;border-radius:8px;
        padding:8px 10px;font-size:12px;font-weight:700;color:#059669;margin-top:4px;">
  ...
  <div style="font-family:'Hind Siliguri',Arial,sans-serif;padding:16px;color:#1a202c;max-width:400px;">
    ...
    <div style="text-align:center;font-size:10px;color:#94a3b8;padding-top:8px;
      border-top:1px dashed #e2e8f0;">
      NovaTEch BD · ধন্যবাদ আমাদের সাথে ব্যবসার জন্য 🙏
    </div>
...
window.shareInvoiceWA=()=>{
  // ✅ Formatted WhatsApp message — plain text থেকে সুন্দর
  const ic=$('invoiceContent');if(!ic)return;

  // Invoice content থেকে sale data বের করি
  const invNoEl = ic.querySelector('[style*="letter-spacing:1px"]');
  const invNo = invNoEl ? invNoEl.textContent.trim() : '';

  // সব text নিই এবং পরিষ্কার করি
  const rawText = ic.innerText || '';
  // formatted message বানাই
...
function syncGlobals(){
  ...
  window._update=update; window._remove=remove;
