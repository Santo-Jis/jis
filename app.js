...
const san = (...a) => window.san(...a);
const validateImage = (...a) => window.validateImage(...a);
const validateDoc = (...a) => window.validateDoc(...a);
const validateMultiDocs = (...a) => window.validateMultiDocs(...a);
...
function initApp(){
  ...
  // ✅ Phone Edit requests listener
  onValue(ref(db,'phoneEditRequests'),s=>{
  ...
  if(CR === 'admin' && typeof window._loadEruda === 'function') {
    window._loadEruda();
  }
...
async function initFCMPushNotification() {
  try {
    ...
    onValue(ref(db, 'notifications/' + CU.uid), snap => {
      const data = snap.val();
      if (!data) return;
  ...
  try {
    ...
    for (const sale of pending) {
      ...
      try {
        ...
        const _snap = await get(ref(db, 'invoiceCounter'));
        const _count = (_snap.exists() ? (_snap.val()||0) : 0) + 1;
        await set(ref(db, 'invoiceCounter'), _count);
...
function loadCustomerSelect(){
  ...
  if(activeRouteId){
    const routeName=allRoutes[activeRouteId]?.name||'';
  ...
  $('cdBody').innerHTML=`
    ...
    <div style="background:var(--card);border-radius:10px;padding:13px;border:1px solid var(--border);margin-bottom:10px;">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        ...
        <button onclick="openEditCustPhone('${id}')" style="font-size:10px;padding:3px 8px;background:rgba(59,130,246,.1);border:1px solid var(--blue);color:var(--blue);border-radius:6px;cursor:pointer;font-family:inherit;">✏️ সম্পাদনা</button>
      </div>
...
// ══════════════════════════════════════════════════
//  📱 Customer Phone Edit Request System
//  Worker আবেদন করে → সাথে সাথে effective
//  Admin/Manager approve → log-এ confirm
//  Admin/Manager reject → পুরনো নম্বর rollback
// ══════════════════════════════════════════════════

// আবেদন করা (সবাই পারবে)
window.openEditCustPhone = async (custId) => {
  const c = allCustomers[custId]; if (!c) return;

  const newSms = prompt(
    `📨 SMS নম্বর পরিবর্তন:\nবর্তমান: ${c.smsNum || 'নেই'}\nনতুন নম্বর লিখুন:`,
    c.smsNum || ''
  );
  if (newSms === null) return; // cancel

  const newWa = prompt(
    `📱 WhatsApp নম্বর পরিবর্তন:\nবর্তমান: ${c.waNum || 'নেই'}\nনতুন নম্বর লিখুন:`,
    c.waNum || ''
  );
  if (newWa === null) return; // cancel

  // validation
  const fmtSms = window.formatPhone(newSms.trim());
  const fmtWa  = window.formatPhone(newWa.trim());
  if (newSms.trim() && !window.validatePhone(fmtSms)) {
    showToast('❌ SMS নম্বর সঠিক নয়! (যেমন: 01712345678)', true); return;
  }
  if (newWa.trim() && !window.validatePhone(fmtWa)) {
    showToast('❌ WhatsApp নম্বর সঠিক নয়! (যেমন: 01712345678)', true); return;
  }

  const finalSms = newSms.trim() ? fmtSms : (c.smsNum || '');
  const finalWa  = newWa.trim()  ? fmtWa  : (c.waNum  || '');

  // একই নম্বর হলে কিছু করার নেই
  if (finalSms === (c.smsNum || '') && finalWa === (c.waNum || '')) {
    showToast('নম্বর পরিবর্তন হয়নি', true); return;
  }
  ...
  try {
    ...
    await update(ref(db, 'customers/' + custId), {
      smsNum: finalSms,
      waNum:  finalWa,
    });

    // ✅ ২. Request log তৈরি করি
    await push(ref(db, 'phoneEditRequests'), {
      custId,
      custName:    c.name,
      oldSmsNum:   c.smsNum || '',
      oldWaNum:    c.waNum  || '',
    ...
    // ✅ ৩. Admin/Manager কে নোটিফিকেশন
    if (CR === 'worker') {
      window.sendNotificationToRole && window.sendNotificationToRole(
        'manager',
        '📱 নম্বর পরিবর্তনের আবেদন',
        `${CN} — ${c.name} এর নম্বর পরিবর্তন করেছে। অনুমোদন করুন।`,
        'cust'
      );
    }

    showToast('✅ নম্বর আপডেট হয়েছে · অনুমোদন পেন্ডিং');
    setTimeout(() => viewCustomer(custId), 300);

  } catch (e) {
    showToast('আবেদন ব্যর্থ: ' + e.message, true);
  } finally {
    loader(false);
  }
};

// ✅ Approve — শুধু log আপডেট
window.approvePhoneEditRequest = async (reqId) => {
  const req = allPhoneEditRequests[reqId]; if (!req) return;
  try {
    await update(ref(db, 'phoneEditRequests/' + reqId), {
      status:       'approved',
      approvedBy:    CN,
...
// ✅ Reject — পুরনো নম্বর rollback
window.rejectPhoneEditRequest = async (reqId) => {
  const req = allPhoneEditRequests[reqId]; if (!req) return;
  if (!confirm(`"${req.custName}" এর নম্বর পরিবর্তন বাতিল করবেন?\nপুরনো নম্বর ফিরে আসবে।`)) return;
  try {
    // ১. পুরনো নম্বর ফেরত দাও (rollback)
    await update(ref(db, 'customers/' + req.custId), {
      smsNum: req.oldSmsNum,
      waNum:  req.oldWaNum,
    });
    // ২. Request বাতিল mark করো
    await update(ref(db, 'phoneEditRequests/' + reqId), {
      status:       'rejected',
      rejectedBy:    CN,
    ...
    window.sendNotificationTo && window.sendNotificationTo(
      req.requestedByUid,
      '❌ নম্বর পরিবর্তন বাতিল',
      `${req.custName} এর নম্বর পরিবর্তনের আবেদন বাতিল করা হয়েছে। পুরনো নম্বর ফিরে এসেছে।`,
      'cust'
    );
    showToast('❌ বাতিল · পুরনো নম্বর ফিরে এসেছে');
    renderPhoneEditRequests();
  } catch (e) { showToast('ব্যর্থ: ' + e.message, true); }
};

// ✅ Render — Admin/Manager এর জন্য
function renderPhoneEditRequests() {
  ...
  const statusTag = (r) => {
    if (r.status === 'approved') return `<span style="font-size:9px;background:rgba(16,185,129,.15);color:var(--green);padding:2px 8px;border-radius:5px;border:1px solid var(--green);font-weight:700;">✅ অনুমোদিত</span>`;
    if (r.status === 'rejected') return `<span style="font-size:9px;background:rgba(239,68,68,.1);color:var(--red);padding:2px 8px;border-radius:5px;border:1px solid var(--red);font-weight:700;">❌ বাতিল</span>`;
    return `<span style="font-size:9px;background:rgba(245,158,11,.15);color:var(--accent);padding:2px 8px;border-radius:5px;border:1px solid var(--accent);font-weight:700;">⏳ অপেক্ষমান</span>`;
  };
  ...
  const card = (id, r, showActions) => `
    <div style="background:var(--card);border:1px solid ${r.status==='pending'?'rgba(245,158,11,.3)':r.status==='approved'?'rgba(16,185,129,.2)':'rgba(239,68,68,.2)'};border-radius:12px;padding:13px;margin-bottom:10px;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
        <div>
          <div style="font-size:13px;font-weight:700;">🏪 ${san(r.custName)}</div>
          <div style="font-size:11px;color:var(--muted);margin-top:3px;">
            👤 আবেদন: ${san(r.requestedBy)} (${r.requestedByRole==='worker'?'কর্মী':'ম্যানেজার'})
            · ${new Date(r.requestedAt).toLocaleString('bn-BD')}
          </div>
          ${r.approvedBy ? `<div style="font-size:11px;color:var(--green);margin-top:2px;">✅ অনুমোদন: ${san(r.approvedBy)} · ${new Date(r.approvedAt).toLocaleString('bn-BD')}</div>` : ''}
          ${r.rejectedBy ? `<div style="font-size:11px;color:var(--red);margin-top:2px;">❌ বাতিল: ${san(r.rejectedBy)} · ${new Date(r.rejectedAt).toLocaleString('bn-BD')}</div>` : ''}
        </div>
        ${statusTag(r)}
      </div>
      <div style="background:var(--surface);border-radius:8px;padding:9px;margin-bottom:${showActions?'10px':'0'};font-size:12px;">
        <div style="margin-bottom:4px;">
          ...
          <span style="color:var(--red);text-decoration:line-through;margin:0 5px;">${san(r.oldSmsNum||'নেই')}</span>
          <span style="color:var(--muted);">→</span>
          <span style="color:var(--green);margin-left:5px;font-weight:700;">${san(r.newSmsNum||'নেই')}</span>
        </div>
        <div>
          <span style="color:var(--muted);">📱 WA:</span>
          <span style="color:var(--red);text-decoration:line-through;margin:0 5px;">${san(r.oldWaNum||'নেই')}</span>
          <span style="color:var(--muted);">→</span>
          <span style="color:var(--green);margin-left:5px;font-weight:700;">${san(r.newWaNum||'নেই')}</span>
        </div>
      </div>
      ${showActions ? `
      <div style="display:flex;gap:8px;">
        <button onclick="approvePhoneEditRequest('${id}')"
          ...
          color:var(--green);border-radius:8px;cursor:pointer;font-family:inherit;font-size:12px;font-weight:700;">
          ✅ অনুমোদন
        </button>
        <button onclick="rejectPhoneEditRequest('${id}')"
          style="flex:1;padding:8px;background:rgba(239,68,68,.08);border:1px solid var(--red);
          color:var(--red);border-radius:8px;cursor:pointer;font-family:inherit;font-size:12px;">
          ❌ বাতিল (rollback)
        </button>
      </div>` : ''}
    </div>`;
...
window.addSale=async()=>{
  ...
  if(pay==='partial'){
    if(!part||part<=0){showToast('আংশিক পরিমাণ লিখুন!',true);return;}
    if(part>=total){showToast('আংশিক পরিমাণ মোটের চেয়ে কম হতে হবে!',true);return;}
  }
  ...
  if(!navigator.onLine){
    ...
    try {
      const _year = new Date().getFullYear();
      const _invoiceSnap = await get(ref(db, 'invoiceCounter'));
      const _lastCount = _invoiceSnap.exists() ? (_invoiceSnap.val() || 0) : 0;
      const _newCount = _lastCount + 1;
      await set(ref(db, 'invoiceCounter'), _newCount);
...
window.generateInvoice=(saleId)=>{
  ...
  <div style="font-family:'Hind Siliguri',Arial,sans-serif;padding:16px;color:#1a202c;max-width:400px;">
    ...
    <div style="display:flex;justify-content:space-between;margin-bottom:12px;
      ...
      <div>
        <div style="font-size:10px;color:#64748b;margin-bottom:2px;">দোকান</div>
        <div style="font-weight:700;font-size:14px;">${s.shop||'–'}</div>
      </div>
      <div style="text-align:right;">
