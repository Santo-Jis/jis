...
window.goBack = function() {
  if (_pageHistory.length > 1) {
    _pageHistory.pop(); // current সরাই
    const prev = _pageHistory[_pageHistory.length - 1];
    showPage(prev, true); // true = back action
  }
};
...
window.showPage=id=>{
  ...
  if(id !== _currentPage) {
    ...
    if (_pageHistory.length > 15) _pageHistory.shift();
    _currentPage = id;
    // Back বাটন দেখাই/লুকাই
    const backBtn = document.getElementById('globalBackBtn');
    if (backBtn) backBtn.style.display = (id !== 'dash') ? 'flex' : 'none';
  }
  // Bottom tab active
...
window.deleteWorkerDoc=async(uid, docIndex)=>{
  ...
  if(!confirm('"'+doc.name+'" ডকুমেন্টটি সম্পূর্ণরূপে মুছে ফেলবেন?

⚠️ Firebase Database থেকে সম্পূর্ণ মুছে যাবে।'))return;
...
// প্রতিটি showPage-এ history state push করি
