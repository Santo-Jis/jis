...
// Firebase Storage সরানো হয়েছে — Cloudinary ব্যবহার করা হচ্ছে
// FCM messaging removed — Spark plan compatible in-app notification used instead
...
function initApp(){
  ...
  onValue(ref(db,'users'),s=>{allUsers=s.val()||{};syncGlobals();if(CR==='admin'||CR==='manager')renderUserList();loadAllWorkerSelects();});
    ...
    <div style="background:var(--card);border-radius:12px;padding:13px;border:1px solid var(--border);margin-bottom:10px;">
      ...
      ${docs.length===0
        ...
        : docs.map(d=>`
          <a href="${d.url}" target="_blank"
            style="display:flex;align-items:center;gap:10px;padding:10px;background:var(--surface);border-radius:10px;margin-bottom:7px;text-decoration:none;color:var(--text);border:1px solid var(--border);">
            <div style="width:36px;height:36px;border-radius:8px;background:rgba(74,158,255,.15);border:1px solid var(--blue);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">
              ${d.name&&d.name.endsWith('.pdf')?'📄':d.name&&(d.name.endsWith('.jpg')||d.name.endsWith('.png'))?'🖼️':'📎'}
            </div>
            <div style="flex:1;min-width:0;">
              <div style="font-size:13px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${d.name||'ফাইল'}</div>
              <div style="font-size:11px;color:var(--blue);margin-top:2px;">Google Drive এ দেখুন →</div>
            </div>
            <div style="font-size:18px;color:var(--muted);">›</div>
          </a>`).join('')
      }
      <!-- Admin নতুন ডকুমেন্ট আপলোড করতে পারবে -->
      <label style="display:flex;align-items:center;gap:8px;padding:10px;background:rgba(245,166,35,.06);border:1px dashed rgba(245,166,35,.3);border-radius:10px;cursor:pointer;margin-top:5px;"
        ...
        <div>
          ...
          <div style="font-size:10px;color:var(--muted);">PDF, ছবি, Word</div>
        </div>
      </label>
