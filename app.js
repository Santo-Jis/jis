...
// ── analytics.js যাতে access করতে পারে
function syncGlobals(){
  window.allSales=allSales;window.allExpenses=allExpenses;
  window.allProducts=allProducts;window.allUsers=allUsers;
  window.allCustomers=allCustomers;window.allRoutes=allRoutes;
  window.allStock=allStock;window.allStockAssign=allStockAssign;
  window.allAttendance=allAttendance;window.allSalaries=allSalaries;
  window.allCommConfig=allCommConfig;window.CR=CR;window.CU=CU;
}
...
// ── Debounce — ঘন ঘন call আটকায়
function debounce(fn, ms=200){
  let t; return (...args)=>{clearTimeout(t);t=setTimeout(()=>fn(...args),ms);};
}
const dRefreshDash=debounce(()=>_refreshDash(),150);
const dRenderReport=debounce(()=>_renderReport(),300);
...
// ── Loader timeout — ১০ সেকেন্ড পরে আটকে থাকলে auth screen দেখাবে
const loaderTimeout = setTimeout(()=>{
  if($('loader')?.style.display !== 'none'){
    loader(false);
    $('authScreen').style.display='flex';
    showErr('লোড হতে সমস্যা হচ্ছে। পেজ refresh করুন।');
  }
}, 10000);
...
onAuthStateChanged(auth,async user=>{
  ...
  if(user){
    ...
    try{
      ...
      loader(false);
      $('authScreen').style.display='flex';
      showErr('সংযোগ সমস্যা: '+getErrMsg(e));
    }
  }else{
...
function initApp(){
  ...
  // Priority 1: Critical data (dashboard দেখাতে লাগে) — সাথে সাথে
  // Priority 2: Page-specific data — page খুললে তখন
  // Priority 3: Background data — ৩ সেকেন্ড পরে

  // ── Priority 1: সাথে সাথে লোড (dash দেখাতে লাগে)
  const p1 = [
    // Sales — শুধু এই মাসের (সব নয়)
    CR==='worker'
      ? ()=>listen(ref(db,'sales'),s=>{const all=s.val()||{};allSales={};Object.entries(all).forEach(([k,v])=>{if(v.uid===CU.uid)allSales[k]=v;});dRefreshDash();renderSaleList();renderDue();renderProfile();})
      : ()=>listen(ref(db,'sales'),s=>{allSales=s.val()||{};dRefreshDash();renderSaleList();renderDue();renderProfile();checkLowStockAlert();}),
    ...
    ()=>listen(ref(db,'routes'),s=>{allRoutes=s.val()||{};renderRouteChips();loadRouteSelects();}),
  ];

  // ── Priority 2: Page খুললে তখন লোড হবে
  window._lazyListeners = {
    expenses: ()=>{
      if(window._loadedExpenses)return;
      window._loadedExpenses=true;
      CR==='worker'
        ? listen(ref(db,'expenses'),s=>{const all=s.val()||{};allExpenses={};Object.entries(all).forEach(([k,v])=>{if(v.uid===CU.uid)allExpenses[k]=v;});dRefreshDash();renderExpList();})
        : listen(ref(db,'expenses'),s=>{allExpenses=s.val()||{};dRefreshDash();renderExpList();if(CR==='admin')renderReport();renderExpenseApprovals();});
    },
    customers: ()=>{
      if(window._loadedCustomers)return;
      window._loadedCustomers=true;
      listen(ref(db,'customers'),s=>{allCustomers=s.val()||{};renderCustomers();loadCustomerSelect();loadBroadcastRoutes();});
    },
    attendance: ()=>{
      if(window._loadedAttendance)return;
      window._loadedAttendance=true;
      CR==='worker'
        ? listen(ref(db,'attendance'),s=>{const all=s.val()||{};allAttendance={};Object.entries(all).forEach(([k,v])=>{if(v.uid===CU.uid)allAttendance[k]=v;});renderAttendance();})
        : listen(ref(db,'attendance'),s=>{allAttendance=s.val()||{};renderAttendance();checkLateAlert();});
    },
    leaves: ()=>{
      if(window._loadedLeaves)return;
      window._loadedLeaves=true;
      CR==='worker'
        ? listen(ref(db,'leaves'),s=>{const all=s.val()||{};allLeaves={};Object.entries(all).forEach(([k,v])=>{if(v.uid===CU.uid)allLeaves[k]=v;});renderLeaves();})
        : listen(ref(db,'leaves'),s=>{allLeaves=s.val()||{};renderLeaves();});
    },
    stock: ()=>{
      if(window._loadedStock)return;
      window._loadedStock=true;
    ...
    salary: ()=>{
      if(window._loadedSalary)return;
      window._loadedSalary=true;
      CR==='worker'
        ? listen(ref(db,'salaries/'+CU.uid),s=>{allSalaries={};if(s.exists())allSalaries[CU.uid]=s.val();renderProfile();})
        : listen(ref(db,'salaries'),s=>{allSalaries=s.val()||{};renderSalary();});
    },
    report: ()=>{
      if(window._loadedReport)return;
      window._loadedReport=true;
      listen(ref(db,'commConfig'),s=>{allCommConfig=s.val()||getDefaultSlabs();renderCommSlabs();});
    },
    admin: ()=>{
      if(window._loadedAdmin)return;
      window._loadedAdmin=true;
      listen(ref(db,'users'),s=>{allUsers=s.val()||{};if(CR==='admin')renderUserList();loadAllWorkerSelects();});
      listen(ref(db,'allowances'),s=>{allAllowances=s.val()||{};renderMyAllowance();renderAllowList();});
  ...
  // ── App দেখা যাচ্ছে, loader বন্ধ করি
  loader(false); showPage('dash');
  startSessionTimer();
  patchAuditLogs();
  renderNotifBell();
  if(!isOnline)showOfflineBanner(true);
  if(offlineQueue.length)setTimeout(flushQueue,2000);

  // ── ৩ সেকেন্ড পরে background data (routes/stock needed for sale page)
  setTimeout(()=>{
    window._lazyListeners.expenses?.();
    window._lazyListeners.customers?.();
  }, 800);

  setTimeout(()=>{
    if(CR!=='worker')window._lazyListeners.admin?.();
    window._lazyListeners.report?.();
  }, 3000);
}
...
window.showPage=id=>{
  ...
  // ── Lazy load data when page opens
  const lazy=window._lazyListeners||{};
  if(id==='exp'||id==='report')lazy.expenses?.();
  if(id==='cust')lazy.customers?.();
  if(id==='att')lazy.attendance?.(),lazy.leaves?.();
  if(id==='stock')lazy.stock?.();
  if(id==='salary')lazy.salary?.(),lazy.report?.();
  if(id==='due')lazy.expenses?.(),lazy.customers?.();
  if(id==='admin')lazy.admin?.();
  if(id==='allow')lazy.admin?.();
  if(id==='teams')lazy.admin?.();
  if(id==='sale')lazy.customers?.(),lazy.stock?.();
  if(id==='route')lazy.customers?.();

  if(id==='report'){renderReport();lazy.report?.();}
  if(id==='salary')renderSalary();
  if(id==='profile'){lazy.salary?.();renderProfile();}
...
window.setFilter=(f,btn)=>{
  ...
  btn.classList.add('active');dRefreshDash();
};
...
function _refreshDash(){
  syncGlobals(); // analytics.js-এর জন্য window-এ sync করি
...
function refreshDash(){dRefreshDash();}
...
window.startRoute=async()=>{
  ...
  showToast('রুট শুরু ✓');renderVisitList();dRefreshDash();
};
