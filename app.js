...
window.saveAIConfig=async()=>{
  ...
  if(!apiKey){showToast('API Key দিন!',true);return;}
  if(!apiKey.startsWith('sk-ant-')){showToast('⚠️ API Key সঠিক নয়! sk-ant- দিয়ে শুরু হওয়া উচিত',true);return;}
  await set(ref(db,'aiConfig'),{apiKey,anthropicApiKey:apiKey,updatedBy:CN,ts:Date.now()});
  // Test করা
  showToast('পরীক্ষা করা হচ্ছে...');
  try{
    ...
    const data=await resp.json();
    if(data.content){
      showToast('✅ Claude AI সংযুক্ত হয়েছে!');
      const status=$('aiConfigStatus');
      if(status)status.innerHTML='<div style="font-size:11px;color:var(--green);margin-top:6px">✅ সংরক্ষিত ও সক্রিয় · Claude AI কাজ করছে</div>';
    } else {
      showToast('⚠️ Key সেভ হয়েছে কিন্তু সংযোগ ব্যর্থ: '+( data.error?.message||'অজানা সমস্যা'),true);
      const status=$('aiConfigStatus');
      if(status)status.innerHTML=`<div style="font-size:11px;color:var(--red);margin-top:6px">⚠️ সেভ হয়েছে কিন্তু সংযোগ সমস্যা: ${data.error?.message||''}</div>`;
    }
  }catch(e){
    showToast('Key সেভ হয়েছে (সংযোগ পরীক্ষা ব্যর্থ)',true);
    const status=$('aiConfigStatus');
    if(status)status.innerHTML='<div style="font-size:11px;color:var(--accent);margin-top:6px">⚠️ Key সেভ হয়েছে — নেটওয়ার্ক সমস্যায় পরীক্ষা করা যায়নি</div>';
  }
};
