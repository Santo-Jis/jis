...
function renderCustomers(){
  ...
  // ✅ UI-03: নাম, মালিক, ফোন নম্বর, unique ID — সব দিয়ে search
  if(q){
    const qNum = q.replace(/\D/g,''); // শুধু সংখ্যা — phone search এর জন্য
    list=list.filter(([id,c])=>{
      const nameMatch  = (c.name||'').toLowerCase().includes(q);
      const ownerMatch = (c.owner||'').toLowerCase().includes(q);
      const phoneMatch = qNum && ((c.waNum||'').replace(/\D/g,'').includes(qNum) || (c.smsNum||'').replace(/\D/g,'').includes(qNum));
      const idMatch    = (c.uniqueId||'').toLowerCase().includes(q);
      return nameMatch || ownerMatch || phoneMatch || idMatch;
    });
  }
  const el=$('custList');
...
function loadCustomerSelect(){
  ...
  // ✅ UI-03: option এ data attributes যোগ — search এর জন্য
  const makeOption=(id,c)=>{
    const phone=(c.waNum||c.smsNum||'').replace(/\D/g,'');
    const uid=c.uniqueId||'';
    const label=c.name+(c.owner?' · '+c.owner:'')+(uid?' ['+uid+']':'');
    return `<option value="${id}" data-name="${(c.name||'').toLowerCase()}" data-phone="${phone}" data-uid="${uid.toLowerCase()}" data-owner="${(c.owner||'').toLowerCase()}">${label}</option>`;
  };
  ...
  if(activeRouteId){
    ...
    if(routeCusts.length){
      ...
      html+=routeCusts.map(([id,c])=>makeOption(id,c)).join('');
      html+=`</optgroup>`;
    }
    if(otherCusts.length){
      html+=`<optgroup label="অন্যান্য দোকান">`;
      html+=otherCusts.map(([id,c])=>makeOption(id,c)).join('');
      html+=`</optgroup>`;
    }
  } else {
    html+=allCusts.map(([id,c])=>makeOption(id,c)).join('');
  }

  html+='<option value="__m__">✏️ ম্যানুয়াল লিখুন</option>';
  sel.innerHTML=html;

  // ✅ search input reset করি
  const inp=$('custSearchInput');
  if(inp) inp.value='';
  filterCustomerSelect('');
}

// ✅ UI-03: Customer Search Filter function
window.filterCustomerSelect = function(q) {
  const sel = $('sShopSel');
  const clearBtn = $('custSearchClear');
  const emptyMsg = $('custSearchEmpty');
  if (!sel) return;

  // Clear button দেখানো/লুকানো
  if (clearBtn) clearBtn.style.display = q ? 'block' : 'none';

  const query = q.toLowerCase().trim();

  // ── Route active থাকলে দুটো group: রুটের দোকান ও অন্যান্য
  // ── Route না থাকলে সব একসাথে
  // ── Search করলে রুটের দোকান আগে দেখাবে, তারপর বাকিগুলো

  let routeCount = 0;   // রুটের মধ্যে match
  let otherCount = 0;   // অন্যান্যের মধ্যে match
  let noRouteCount = 0; // route group নেই এমন match

  Array.from(sel.querySelectorAll('option')).forEach(opt => {
    // placeholder ও manual — সবসময় দেখাই
    if (!opt.value || opt.value === '__m__') {
      opt.style.display = '';
      return;
    }
    ...
    // optgroup এর মধ্যে কোনো match আছে কিনা track করি
    const grp = opt.parentElement;
    if (grp && grp.tagName === 'OPTGROUP') {
      const label = grp.getAttribute('label') || '';
      if (match) {
        if (label.includes('রুট')) routeCount++;
        else otherCount++;
      }
    } else {
      if (match) noRouteCount++;
    }
  });

  // optgroup heading — match না থাকলে hide করি
  Array.from(sel.querySelectorAll('optgroup')).forEach(grp => {
    if (!query) { grp.style.display = ''; return; }
    const label = grp.getAttribute('label') || '';
    const hasVisible = Array.from(grp.querySelectorAll('option'))
      .some(o => o.style.display !== 'none');
    grp.style.display = hasVisible ? '' : 'none';
  });

  const totalMatch = routeCount + otherCount + noRouteCount;

  // "পাওয়া যায়নি" message
  if (emptyMsg) {
    emptyMsg.style.display = (query && totalMatch === 0) ? 'block' : 'none';
  }
  sel.style.display = (query && totalMatch === 0) ? 'none' : 'block';
};

// ✅ UI-03: Search clear করার function
...
function clearSaleForm(){
  ...
  // ✅ UI-03: বিক্রয় শেষে search box clear করি
  const inp=$('custSearchInput');
  if(inp){inp.value='';filterCustomerSelect('');}
}
