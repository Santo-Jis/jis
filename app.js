...
function buildNav(){
  ...
  drawerSections.forEach(sec => {
    ...
    secItems.forEach(g => {
      const idx = globalIdx++;
      const hasSub = !!g.sub;
      // ✅ চ্যাট item এ unread badge
      const _chatBadge = g.page === 'chat'
        ? `<span id="drawerChatBadge" style="
...
window.showPage=function(id, isBack){
  ...
  // ✅ FIX: Chat page — conversation list load + সব message read mark
  if(id==='chat'){
    setTimeout(()=>{
      if(typeof window.ftRenderConvList==='function') window.ftRenderConvList();
    },300);
    // ✅ chat page খুললেই সব unread message read mark → badge clear
    setTimeout(()=>{
