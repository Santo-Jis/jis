...
window.addSale=async()=>{
  ...
  // ✅ Invoice নম্বর তৈরি — NTB-YYYY-XXXX format
  ...
  const invoiceNo = `NTB-${_year}-${String(_newCount).padStart(4,'0')}`;
  ...
  <div style="font-family:'Hind Siliguri',Arial,sans-serif;padding:16px;color:#1a202c;max-width:400px;">
    ...
    <div style="background:#f8fafc;border-radius:8px;padding:10px 12px;margin-bottom:10px;">
      ...
      ${s.due>0?`<div style="display:flex;justify-content:space-between;font-size:13px;
        ...
        <span>বাকি</span><span>${bn(s.due)}</span>
      </div>`:''}
    </div>
...
function checkMonthlyAlerts() {
  ...
  const salaryPaid = Object.values(allSalaries||{}).some(s => s.month === monthKey && s.paid);
  ...
  if (day <= 3 && !localStorage.getItem(shownKey)) {
    ...
    const lmSalaryPaid = Object.values(allSalaries||{}).some(s => s.month===lastMonthKey && s.paid);
  ...
  mo.innerHTML = `
    <div style="background:var(--surface);border-radius:20px 20px 0 0;padding:22px;
      ...
      <div style="margin-bottom:16px;">
        ...
        ${[
          {done: lmSalaryPaid, label: 'কর্মীদের বেতন দেওয়া হয়েছে',     icon: '💰', page: 'salary'},
          {done: false,        label: 'মাসিক রিপোর্ট PDF নামানো হয়েছে', icon: '📊', page: 'report'},
          {done: lmDue===0,    label: 'সব বাকি আদায় হয়েছে',             icon: '🏦', page: 'due'},
        ].map(item => `
          <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;
            background:var(--card);border-radius:10px;margin-bottom:6px;
            border:1px solid var(--border-l);cursor:pointer;"
            onclick="if(!${item.done}){showPage('${item.page}');setActiveTab('${item.page}');document.getElementById('monthlyChecklistModal').remove();}">
