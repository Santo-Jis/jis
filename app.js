...
let allSales={},allExpenses={},allPaymentLogs={},allProducts={},allUsers={},allAllowances={},allCustomers={},allRoutes={},allRouteRequests={},allStock={},allStockAssign={},allAttendance={},allLeaves={},allSalaries={},allCommConfig={},allNotices={},allTeams={},allSMSConfig={},allDeleteRequests={};
...
function renderTeams(){
  ...
  el.innerHTML=teamsToShow.map(([id,t])=>{
    ...
    const membersHTML=members.map(uid=>{
      ...
      const mTotal=mSales.reduce((a,b)=>a+(b.total||0),0);
      const attCount=Object.values(allAttendance).filter(a=>{
      ...
      const todayAtt=Object.values(allAttendance).find(a=>a.uid===uid&&a.date===today());
      const isOnline=todayAtt&&!todayAtt.checkOut;
...
function renderUserList(){
  ...
  $('userList').innerHTML=Object.entries(allUsers).map(([uid,u])=>{
    ...
    const lateCount=Object.values(allAttendance).filter(a=>{
      const d=new Date(a.date);return a.uid===uid&&d.getMonth()===now.getMonth()&&a.isLate;
    }).length;
    // বাকি
    const myDue=(_sc.byUid[uid]||[]).filter(s=>s.due>0).reduce((a,s)=>a+s.due,0);
    // টার্গেট
