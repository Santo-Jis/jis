...
function buildNav(){
  ...
  const drawerGroups = [
    ...
    // ── নোটিশ — admin পেজে scroll করবে
    { ico:'📢', lbl:'নোটিশ', page:'admin', scroll:'noticeTitle', r:['admin','manager'] },
...
window.showPage=function(id, isBack){
  ...
  if(id==='report'){
    renderReport();
    setTimeout(()=>{
      if(typeof drawSalesTrendChart==='function') drawSalesTrendChart();
      if(typeof drawTopProductsChart==='function') drawTopProductsChart();
      if(typeof drawTopShopsChart==='function') drawTopShopsChart();
...
window.renderLedger = () => {
  ...
  const totalCash    = transactions.filter(t => t.type === 'sale').reduce((a, t) => a + t.cash, 0);
    ...
    summaryEl.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
        ...
        <div style="background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.2);border-radius:12px;padding:10px;text-align:center">
          ...
          <div style="font-size:16px;font-weight:800;color:var(--green);font-family:'Sora',sans-serif">৳${Math.round(totalCash + totalPayment).toLocaleString('bn-BD')}</div>
        </div>
