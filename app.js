...
window._uploadImageToFirebase = uploadImageToFirebase;
window._db      = db;
window._ref     = ref;
window._push    = push;
window._set     = set;
...
function buildNav(){
  const tabs=[
    ...
    {id:'tasks',     label:'📋 টাস্ক',         r:['admin','manager','worker']},
    {id:'chat',      label:'💬 চ্যাট',          r:['admin','manager','worker']},
    {id:'cust',      label:'🏪 কাস্টমার',       r:['admin','manager','worker']},
    {id:'route',     label:'🗺️ রুট',            r:['admin','manager','worker']},
    {id:'beat',      label:'🗺️ বিট প্ল্যান',    r:['worker','manager','admin']},
    {id:'sale',      label:'🛍 বিক্রয়',          r:['admin','manager','worker']},
    {id:'order',     label:'🛒 অর্ডার',          r:['admin','manager','worker']},
    ...
    {id:'leader',    label:'🏆 র‌্যাংকিং',       r:['admin','manager','worker']},
    {id:'meeting',   label:'📅 মিটিং',           r:['admin','manager','worker']},
    {id:'forms',     label:'📋 ফর্ম',            r:['admin','manager','worker']},
    {id:'salary',    label:'💰 বেতন',            r:['admin','manager']},
    {id:'payroll',   label:'💰 Payroll',         r:['admin']},
    {id:'approve',   label:'✅ অনুমোদন',         r:['admin','manager']},
    {id:'track',     label:'📡 ট্র্যাকিং',        r:['admin','manager']},
    {id:'mileage',   label:'🛣️ মাইলেজ',          r:['admin','manager']},
    {id:'analytics', label:'📊 Analytics',       r:['admin']},
    {id:'territory', label:'🗺️ টেরিটরি',         r:['admin']},
    {id:'holiday',   label:'📅 ছুটি',            r:['admin']},
    {id:'exppolicy', label:'💸 খরচ নীতি',        r:['admin']},
    {id:'teams',     label:'👥 টিম',             r:['admin','manager']},
    {id:'profile',   label:'👤 প্রোফাইল',        r:['admin','manager','worker']},
    {id:'report',    label:'📊 রিপোর্ট',          r:['admin']},
    {id:'folders',   label:'🗂️ ফোল্ডার',         r:['admin']},
...
window.showPage=id=>{
  ...
  // ── নতুন পেজ ──
  if(id==='tasks')setTimeout(()=>window.renderTaskPage?.(),100);
  if(id==='chat')setTimeout(()=>window.renderChatPage?.(),100);
  if(id==='track')setTimeout(()=>window.renderLiveTracking?.(),100);
  if(id==='beat'){setTimeout(()=>{window.loadAllUpgradeSelects?.();window.renderBeatPlan?.();},100);}
  if(id==='leader')setTimeout(()=>window.renderLeaderboard?.(),100);
  if(id==='approve')setTimeout(()=>window.renderApprovalCenter?.(),100);
  if(id==='forms')setTimeout(()=>window.renderCustomFormsPage?.(),100);
  if(id==='order'){setTimeout(()=>{window.loadAllUpgradeSelects?.();window.renderOrderPage?.();},100);}
  if(id==='payroll')setTimeout(()=>window.renderPayrollEngine?.(),100);
  if(id==='analytics')setTimeout(()=>window.renderAnalyticsReports?.(),100);
  if(id==='territory')setTimeout(()=>window.renderTerritoryPage?.(),100);
  if(id==='meeting')setTimeout(()=>window.renderMeetingScheduler?.(),100);
  if(id==='mileage')setTimeout(()=>window.renderMileageReport?.(),100);
  if(id==='holiday')setTimeout(()=>window.renderHolidayList?.(),100);
};
...
function renderDue(){
  ...
  const canPay=(CR==='admin'||CR==='manager');
  $('dueList').innerHTML=Object.keys(dm).length?Object.keys(dm).map(shop=>{
    ...
    return `<div class="due-card">
      ...
      ${canPay?`<button class="pay-btn" onclick="openPayMo('${shop}',${d.total})">💰 পেমেন্ট গ্রহণ</button>`:''}
    </div>`;
...
window.resetAllExpenses=async()=>{
  if(!confirm('সব খরচের হিসাব ০ করে দেবেন? এটি পূর্বাবস্থায় ফেরানো যাবে না।'))return;
  await remove(ref(db,'expenses'));showToast('সব খরচ রিসেট ✓');
};
...
window.switchFolderTab=(tab,btn)=>{
  document.querySelectorAll('#page-folders .fb').forEach(b=>b.classList.remove('active'));
  if(btn)btn.classList.add('active');
  if(typeof renderWorkerFolders==='function'&&tab==='workers')renderWorkerFolders();
  else if(typeof renderCustomerFolders==='function'&&tab==='customers')renderCustomerFolders();
  else if(typeof renderDailyReport==='function'&&tab==='daily')renderDailyReport();
  else if(typeof renderMonthlyReport==='function'&&tab==='monthly')renderMonthlyReport();
};
