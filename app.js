...
window.goBack = function() {
  if (_pageHistory.length > 1) {
    _pageHistory.pop(); // current page সরাই
    const prev = _pageHistory[_pageHistory.length - 1]; // ঠিক আগের page
    showPage(prev, true); // back=true flag
  } else {
    showPage('dash', true); // fallback: dash এ যাও
  }
};
...
window.showPage=id=>{
  ...
  // ✅ History tracking
  const isBack = (arguments[1] === true);
  if (!isBack) {
    // Forward navigation — history তে add করি
