...
import { getDatabase, ref, set, push, get, onValue, update, remove, runTransaction } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js"; // ✅ BUG-02 FIX
...
let pendingSaleData=null,pendingOTP=null,pendingSaleId=null,pendingOTPExpiry=0; // ✅ BUG-05 FIX
  ...
  try {
    ...
    snap.forEach(userSnap => {
      userSnap.forEach(dateSnap => {
        if (new Date(dateSnap.key.replace(/_/g,'-')) < cutoff) {
          updates['gpsTrail/'+userSnap.key+'/'+dateSnap.key] = null;
          deleted++;
        }
      });
    });
  ...
  try {
    ...
    for (const sale of pending) {
      ...
      try {
        ...
        // ✅ BUG-02 FIX: runTransaction offline sync
        const _year = new Date().getFullYear();
        let _count = 1;
        await runTransaction(ref(db, 'invoiceCounter'), (cur) => {
          _count = (cur || 0) + 1;
          return _count;
        });
...
function buildNav(){
  ...
  drawerSections.forEach(sec => {
    ...
    secItems.forEach(g => {
      const idx = globalIdx++;
      const hasSub = !!g.sub;
      const _chatBadge = g.page === 'chat'
        ? `<span id="drawerChatBadge" style="position:absolute;top:4px;right:4px;background:var(--red);color:#fff;font-size:9px;font-weight:700;min-width:16px;height:16px;border-radius:100px;padding:0 4px;display:none;align-items:center;justify-content:center;"></span>`
        : '';
...
window.showPage=function(id, isBack){
  ...
  // ✅ FIX: Chat page — conversation list + read mark
  if(id==='chat'){
    setTimeout(()=>{
      if(typeof window.ftRenderConvList==='function') window.ftRenderConvList();
    },300);
    setTimeout(()=>{
      if(typeof window._ftMarkAllRead==='function') window._ftMarkAllRead();
    },600);
  }
  if(id==='notice'){
...
function sendSMSAlphaNet(phone,msg){
  ...
  fetch('https://api.alphanetkbd.com/api/v2/sending/messages',{
    ...
    body:new URLSearchParams({apikey:allSMSConfig.apiKey,msg:msg,to:num,from:allSMSConfig.senderId||'NOVATECH'})
  }).then(r=>r.json()).then(d=>console.log('SMS sent:',d?.status||d)).catch(e=>console.log('SMS error:',e.message));
}
  ...
  body.innerHTML=custSales.length?custSales.map(s=>`
    <div style="background:var(--surface);border-radius:10px;padding:10px;margin-bottom:8px;border:1px solid var(--border)">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:13px;font-weight:600">🛍 ${san(s.product)}</span>
...
window.viewCust=id=>{
  ...
  $('cdBody').innerHTML=`
    ...
    ${cs.slice(0,5).map(s=>`
      <div class="ec">
        <div class="ei">
          <div class="shop">${san(s.product)} × ${s.qty}</div>
...
window.addSale=async()=>{
  ...
  if(!navigator.onLine){
    ...
    try {
      // ✅ BUG-02 FIX: runTransaction — duplicate invoice বন্ধ
      const _year = new Date().getFullYear();
      let _newCount = 1;
      await runTransaction(ref(db, 'invoiceCounter'), (cur) => {
        _newCount = (cur || 0) + 1;
...
{otp_line}`)
  ...
  if(smsNum&&hasAPI&&otpEnabled){
    ...
    pendingOTPExpiry=Date.now()+(3*60*1000); // ✅ BUG-05 FIX: ৩ মিনিট
...
function loadAllWorkerSelects(){
  ['alWorker','salWorker','asWorker','teamLeader','addTeamMember','addTeamSel'].forEach(sid=>{
    ...
    sel.innerHTML='<option value="">-- বেছে নিন --</option>'+
      Object.entries(allUsers).filter(([,u])=>u.role==='worker'||u.role==='manager').map(([uid,u])=>`<option value="${uid}">${san(u.name)} (${u.role==='worker'?'কর্মী':'ম্যানেজার'})</option>`).join('');
  });
}
...
function renderWorkerRouteStatus() {
  ...
  el.innerHTML=workerEntries.map(([uid,u])=>{
    ...
    return`<div style="background:var(--card);border-radius:var(--r);border:1px solid var(--border-l);
      ...
      <!-- তথ্য -->
      <div style="flex:1;min-width:0;">
        <div style="font-size:13px;font-weight:700;">${san(u.name)}</div>
        ${isToday&&route?`
...
window.downloadSalesPDF=()=>{
  ...
  const rows=sales.map(s=>`<tr><td>${fmtDate(s.date)}</td><td>${san(s.shop)}</td><td>${san(s.product)}</td><td>${s.qty}</td><td>${bn(s.total)}</td><td>${s.payStatus==='paid'?'পরিশোধ':s.payStatus==='due'?'বাকি':'আংশিক'}</td><td>${s.workerName||''}</td></tr>`).join('');
...
window.printWorkerReport=(uid)=>{
  ...
  ${mSales.map(s=>`<tr><td>${s.date}</td><td>${san(s.shop)}</td><td>${san(s.product)}</td><td>${s.qty}</td><td>৳${Math.round(s.total)}</td><td>${s.payStatus}</td></tr>`).join('')}
  </table>
  <h3>খরচ তালিকা</h3>
...
function syncGlobals(){
  ...
  // ✅ BUG-01 FIX: আগে define হয়ে থাকলে আবার define করব না
...
window.sendAIChat=async()=>{
  ...
  msgs.scrollTop=msgs.scrollHeight;
  // ✅ BUG-04 FIX: Firebase থেকে key নিই
  try{
