...
function getFilteredSales(){
  const now=new Date();
  // ✅ Cache থেকে শুরু করি — Object.values() call নেই
  let base = CR==='worker' ? (_sc.byUid[CU?.uid]||[]) : _sc.arr;
  if(filterMode==='today') return base.filter(s=>s.date===today());
  if(filterMode==='week'){
    const st=new Date(now);st.setDate(now.getDate()-6);st.setHours(0,0,0,0);
    return base.filter(s=>new Date(s.date)>=st);
  }
  if(filterMode==='month') return CR==='worker'
    ? base.filter(s=>{const d=new Date(s.date);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();})
    : _sc.thisMonth.slice(); // এই মাস cache থেকে সরাসরি
  return base;
}
...
function renderMyTarget(){
  ...
  if(!target)return;
  const now=new Date();
  const mySales=(_sc.byUid[CU?.uid]||[]).filter(s=>_sc.thisMonth.some(m=>m._id===s._id));
