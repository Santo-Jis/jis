...
window.showPage=function(id, isBack){
  ...
  if(id==='dash') setTimeout(refreshDash, 100);
...
window.saveAIConfig=window.saveAIConfigDebug=async()=>{
  ...
  try{
    ...
    localStorage.setItem('nt-ai-key', apiKey); // ✅ FIX: localStorage তেও save
...
function renderSearchResults(q) {
  ...
  let items = SEARCH_ITEMS.filter(it => it.r.includes(CR));

  if (query.length === 0) {
    // খালি হলে — জনপ্রিয় শর্টকাট দেখাই
    renderSearchGroups(el, [
      { label: '⚡ দ্রুত অ্যাক্সেস', items: items.slice(0, 8) }
    ]);
    return;
  }

  // সার্চ লজিক — title, desc, keys মিলাই
  const matched = items.filter(it => {
    const haystack = [it.title, it.desc, ...(it.keys||[])].join(' ').toLowerCase();
  ...
  if (!matched.length) {
    el.innerHTML = `<div style="text-align:center;padding:32px 16px;color:var(--muted);">
      ...
      <div style="font-size:12px;margin-top:6px;opacity:.6;">অন্য শব্দে চেষ্টা করুন</div>
    </div>`;
    return;
  }

  renderSearchGroups(el, [{ label: `${matched.length}টি রেজাল্ট`, items: matched }]);
}

function renderSearchGroups(el, groups) {
  el.innerHTML = groups.map(g => `
    <div style="padding:6px 0 4px;">
      <div style="font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);padding:0 4px 6px;">${g.label}</div>
      ${g.items.map(it => `
        <div onclick="goSearchItem(${JSON.stringify(it).replace(/"/g,'&quot;')})"
          style="display:flex;align-items:center;gap:12px;padding:11px 10px;border-radius:12px;cursor:pointer;transition:background .12s;margin-bottom:2px;"
          onmouseover="this.style.background='rgba(255,255,255,.05)'"
          onmouseout="this.style.background='transparent'">
          <div style="width:38px;height:38px;border-radius:10px;background:var(--card);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">${it.ico}</div>
          <div style="flex:1;min-width:0;">
            <div style="font-size:14px;font-weight:600;color:var(--text);">${it.title}</div>
            <div style="font-size:12px;color:var(--muted);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${it.desc}</div>
          </div>
...
window.renderLedger = () => {
  ...
  const totalProfit  = transactions.filter(t => t.type === 'sale').reduce((a, t) => a + (t.profit || 0), 0);
  ...
  if (summaryEl) {
    summaryEl.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
        ...
        <div style="background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.3);border-radius:12px;padding:10px;text-align:center">
...
window.ledgerExportExcel = () => {
  ...
  // বিক্রয়
  csv += '--- বিক্রয় তালিকা ---\n';
  csv += 'তারিখ,সময়,দোকান,পণ্য,পরিমাণ,মূল্য,ডিসকাউন্ট%,মোট,নগদ,বাকি,কর্মী,OTP\n';
  Object.values(allSales || {}).filter(s => inRange(s.date || '')).sort((a,b)=>(b.ts||0)-(a.ts||0)).forEach(s => {
