...
import { getDatabase, ref, set, push, get, onValue, update, remove, runTransaction } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js"; // ✅ BUG-02 FIX: runTransaction যোগ
...
let pendingSaleData=null,pendingOTP=null,pendingSaleId=null,pendingOTPExpiry=0; // ✅ BUG-05 FIX: OTP expiry
  ...
  try {
    ...
    for (const sale of pending) {
      ...
      try {
        ...
        // ✅ BUG-02 FIX: runTransaction — offline sync race condition বন্ধ
...
// SMS
// ✅ BUG-07 FIX: GET → POST (API key URL এ expose হবে না)
function sendSMSAlphaNet(phone,msg){
  ...
  fetch('https://api.alphanetkbd.com/api/v2/sending/messages',{
    ...
    body:new URLSearchParams({
      apikey:allSMSConfig.apiKey,
      msg:msg,
      to:num,
      from:allSMSConfig.senderId||'NOVATECH'
    })
...
window.addSale=async()=>{
  ...
  if(!navigator.onLine){
    ...
    try {
      // ✅ BUG-02 FIX: runTransaction — race condition বন্ধ
      const _year = new Date().getFullYear();
      let _newCount = 1;
      await runTransaction(ref(db, 'invoiceCounter'), (cur) => {
        _newCount = (cur || 0) + 1;
...
{otp_line}`)
  ...
  if(smsNum&&hasAPI&&otpEnabled){
    ...
    pendingOTPExpiry=Date.now()+(3*60*1000); // ✅ BUG-05 FIX: ৩ মিনিট expiry
...
// ══ AI CHAT ══
// ✅ BUG-04 FIX: Firebase থেকে key নেওয়া হচ্ছে, window._ntAIKey নয়
window.sendAIChat=async()=>{
  ...
  input.value='';
  msgs.innerHTML+='<div style="background:var(--blue);color:#fff;border-radius:12px 12px 2px 12px;padding:8px 12px;font-size:13px;max-width:80%;align-self:flex-end;margin-left:auto;margin-bottom:6px">'+san(msg)+'</div>';
  ...
  msgs.scrollTop=msgs.scrollHeight;
  try{
    // ✅ Firebase থেকে নিরাপদে key নিই
    const snap=await get(ref(db,'aiConfig'));
    if(!snap.exists()){document.getElementById(tid).textContent='❌ Admin → AI Config এ API Key দিন।';return;}
    const apiKey=snap.val().apiKey||snap.val().anthropicApiKey||'';
