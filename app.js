  ...
  el.innerHTML=list.length?list.map(([id,c])=>{
    ...
    return`<div class="cust-card" style="padding:0;overflow:hidden;">
      ...
      <div style="display:flex;gap:0;border-top:1px solid var(--border);">
        <button onclick="showPage('sale');setActiveTab('sale');setTimeout(()=>{const s=$('sShopSel');if(s){Array.from(s.options).forEach(o=>{if(o.value==='${id}')o.selected=true;});calcSaleSummary();}},400)"
