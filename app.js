...
// ✅ XSS Sanitizer — Firebase data innerHTML এ বসানোর আগে escape করে
const san = (str) => {
  if (str == null) return '';
  return String(str)
    .replace(/&/g, '&amp;')
...
// ✅ File Validation — size ও type check
const FILE_LIMIT = {
  IMAGE_MB:    10,               // একটি ছবি সর্বোচ্চ
  DOC_MB:      20,               // একটি ডকুমেন্ট সর্বোচ্চ
  MULTI_DOC_MB: 50,              // একসাথে একাধিক ডকুমেন্ট মোট সর্বোচ্চ
...
function validateImage(file) {
  if (!FILE_LIMIT.ALLOWED_IMG.includes(file.type)) {
    showToast('❌ শুধু JPG, PNG, WEBP, GIF ছবি দেওয়া যাবে!', true);
    return false;
  }
  if (file.size > FILE_LIMIT.IMAGE_MB * 1024 * 1024) {
    showToast(`❌ ছবির সাইজ ${FILE_LIMIT.IMAGE_MB}MB এর বেশি হওয়া যাবে না!`, true);
    return false;
  }
  return true;
}
function validateDoc(file) {
  if (!FILE_LIMIT.ALLOWED_DOC.includes(file.type)) {
    showToast('❌ শুধু PDF, DOC, DOCX, XLS, XLSX, JPG, PNG ফাইল দেওয়া যাবে!', true);
    return false;
  }
  if (file.size > FILE_LIMIT.DOC_MB * 1024 * 1024) {
    showToast(`❌ ডকুমেন্টের সাইজ ${FILE_LIMIT.DOC_MB}MB এর বেশি হওয়া যাবে না!`, true);
    return false;
  }
  return true;
}
function validateMultiDocs(files) {
  // প্রতিটা ফাইল আলাদা চেক
  for (const f of files) {
    if (!validateDoc(f)) return false;
  }
  // মোট সাইজ চেক
  const totalMB = files.reduce((sum, f) => sum + f.size, 0) / (1024 * 1024);
  if (totalMB > FILE_LIMIT.MULTI_DOC_MB) {
    showToast(`❌ মোট ফাইল সাইজ ${FILE_LIMIT.MULTI_DOC_MB}MB এর বেশি হওয়া যাবে না! (বর্তমান: ${totalMB.toFixed(1)}MB)`, true);
    return false;
  }
  return true;
}
...
let allSales={},allExpenses={},allPaymentLogs={},allProducts={},allUsers={},allAllowances={},allCustomers={},allRoutes={},allRouteRequests={},allStock={},allStockAssign={},allAttendance={},allLeaves={},allSalaries={},allCommConfig={},allNotices={},allTeams={},allSMSConfig={},allDeleteRequests={},allWorkerStatus={};
